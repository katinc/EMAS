    ...
    public partial class OverTimeNew : Form
    {
        ...
        private void Clear()
        {
            try
            {
                ...
                cboOverTimeType.DataSource = null;
        ...
        void SelectEmployee(string StaffID)
        {
            foreach (Employee employee in employees)
            {
                if (employee.StaffID.Trim() == StaffID)
                {
        ...
        void searchGrid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                {
                    Clear();
                    SelectEmployee(searchGrid.CurrentRow.Cells["gridStaffNo"].Value.ToString().Trim());
                }
            }
            catch (Exception ex)
            {
        ...
        public void EditOverTimeChange(OverTime overTime)
        {
            try
            {
               ...
               // staffIDtxt_TextChanged(null, EventArgs.Empty); 
                ...
                txtBasicSalary.Text = overTime.BasicSalary.ToString();
                ...
                if (overTime.OverTimeType.Description.ToLower() == "public holiday overtime" && overTime.Holiday.Value != null)
                {
                ...
                SelectEmployee(staffIDtxt.Text.Trim());
        ...
        private void findbtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                overTimeView.btnDelete.Enabled = CanDelete;
                overTimeView.Show();
            }
            catch (Exception ex)
            {
        ...
        private void gradeCategoryTextBox_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                ...
                dalHelper.CreateParameter("@GradeCategory", gradeCategoryTextBox.SelectedItem.ToString(),DbType.String);
                var dtGrades= dalHelper.ExecuteReader("select * from EmployeeGradeView where GradeCategory=@GradeCategory");

                foreach (DataRow dRow in dtGrades.Rows)
                {
                    EmployeeGrade employeeGrade = new EmployeeGrade();
                    employeeGrade.Active = bool.Parse(dRow["Active"].ToString());
                    employeeGrade.Amount = decimal.Parse(dRow["BasicSalary"].ToString());
                    employeeGrade.Archived = bool.Parse(dRow["Archived"].ToString());
                    employeeGrade.Code = dRow["Code"].ToString();
                    employeeGrade.EndStep = (!Information.IsDBNull(dRow["EndStep"])) ? int.Parse(dRow["EndStep"].ToString()) : 0;
                    employeeGrade.Grade = dRow["Description"].ToString();
                    employeeGrade.GradeCategory = gradeCategories[gradeCategoryTextBox.SelectedIndex];
                    employeeGrade.ID = int.Parse(dRow["ID"].ToString());
                    employeeGrade.Level = (!Information.IsDBNull(dRow["GradeLevel"])) ? dRow["GradeLevel"].ToString() : null;
                    employeeGrade.StartStep = (!Information.IsDBNull(dRow["StartStep"])) ? int.Parse(dRow["StartStep"].ToString()) : 0;
                    employeeGrade.User = new User() { ID = int.Parse(dRow["UserID"].ToString()), Name = dRow["UserName"].ToString() };

                    gradess.Add(employeeGrade);
        ...
        private void cboOverTimeType_DropDown(object sender, EventArgs e)
        {
            try
            {
                ...
                cboOverTimeType.DataSource = overTimeTypes;
                cboOverTimeType.DisplayMember = "Description";
                cboOverTimeType.ValueMember = "ID";
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
