...
namespace eMAS.Forms.PayRollProcessing
{
    public partial class OverTimeView : Form
    {
        ...
        void grid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (grid.CurrentRow != null && overTimeNew.CanEdit)
                {
                    ...
                    overTime.Holiday = Convert.ToDateTime(grid.CurrentRow.Cells["gridHoliday"].Value);
                    ...
                    overTime.IsTaxable = bool.Parse(grid.CurrentRow.Cells["gridIsTaxable"].Value.ToString());
                    overTime.Employee.Age = grid.CurrentRow.Cells["gridAge"].Value.ToString();
                    overTime.Employee.Gender = grid.CurrentRow.Cells["gridGender"].Value.ToString();
                    overTime.Employee.ID =int.Parse(grid.CurrentRow.Cells["EmpId"].Value.ToString());
                ...
                if (MessageBox.Show("Are you sure you want to remove " + grid.CurrentRow.Cells["gridStaffName"].Value.ToString() + "'s OverTime?", GlobalData.Caption, MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                {
                    try
                    {
                        if (GlobalData.User.UserCategory.Description == "Intermediate" && GlobalData.User.ID == int.Parse(grid.CurrentRow.Cells["gridUserID"].Value.ToString()))
                        {
                            ...
                            //dal.Delete(overTime);
                            GlobalData.ProcessDelete(this.Name, overTime.ID);
                        ...
                        else if (GlobalData.User.UserCategory.Description == "Basic" || GlobalData.User.UserCategory.Description == "Advance")
                        {
                            ...
                            overTime.ArchiverID = GlobalData.User.ID;
                            //dal.Delete(overTime);
                            GlobalData.ProcessDelete("OverTimeNew", overTime.ID);
                            grid.Rows.RemoveAt(grid.CurrentRow.Index);
                        }
                        else
                        {
        ...
        private void PopulateView(IList<OverTime> overTimes)
        {
            try
            {
                ...
                foreach (OverTime overTime in overTimes)
                {
                    ...
                    grid.Rows[ctr].Cells["gridIsTaxable"].Value = overTime.IsTaxable;
                    grid.Rows[ctr].Cells["gridAge"].Value = overTime.Employee.Age;
                    grid.Rows[ctr].Cells["gridGender"].Value = overTime.Employee.Gender;
                    grid.Rows[ctr].Cells["EmpId"].Value = overTime.Employee.ID;
                    ctr++;
                }
                grid.ClearSelection();
            }
            ...
            try
            {
                ...
                if (dateEntry.Checked==true)
                {
                    query.Criteria.Add(new Criterion()
                    {
                        ...
                        Value = dateEntry.Value.ToString("yyyy-MM-dd"),
                        CriteriaOperator = CriteriaOperator.And
                    });
                }
                if(dateTo.Checked==true){
                    query.Criteria.Add(new Criterion()
                    {
                        Property = "OverTimeView.Date",
                        CriterionOperator = CriterionOperator.LessThanOrEqualTo,
                        Value = dateTo.Value.ToString("yyyy-MM-dd"),
                        CriteriaOperator = CriteriaOperator.And
                    });
                }
