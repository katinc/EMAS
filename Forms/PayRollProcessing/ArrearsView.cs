...
namespace eMAS.Forms.PayRollProcessing
{
    public partial class ArrearsView : Form
    {
        ...
        private Allowance_Arrears allowance_Arrears;
        ...
        public ArrearsView()
        {
            try
            {
                ...
                allowance_Arrears = new Allowance_Arrears();
                this.dal = new DAL();
        ...
        public ArrearsView(IDAL dal, Allowance_Arrears allowanceArrears)
        {
            try
            {
                InitializeComponent();
                this.allowance_Arrears = allowanceArrears;
                this.dal = dal;
                this.arrear = new Arrears();
                this.arrears = new List<Arrears>();
                this.foundArrears = new List<Arrears>();
        ...
        private void grid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (grid.CurrentRow != null && arrearsNew.CanEdit)
                {
                    string type = grid.CurrentRow.Cells["gridType"].Value.ToString();
                    ...
                    if (string.Equals(type, "Promotional") || string.Equals(type, "Normal") || string.Equals(type, "Other"))
                    {
                        if (!arrearsNew.CanEdit)
                        {
                    ...
                    {
                        if (!allowance_Arrears.CanEdit)
                        {
                            MessageBox.Show("Sorry you do not have permission to edit this data.");
                        }
                        else
                        {
                            allowance_Arrears.Arrear = arrear;
                            allowance_Arrears.Show();
                            this.Close();
                        }
                    }
                }
                else if (!arrearsNew.CanEdit)
                {
                    MessageBox.Show("Sorry you do not have permission to edit this data.");
                }
            }
                    ...
                    if (grid.CurrentRow.Cells["gridID"].Value != null)
                    {
                        if (GlobalData.QuestionMessage("Are sure you want to delete this item") == DialogResult.Yes)
                        {
                            ...
                            if (GlobalData.User.UserCategory.Description == "Intermediate" && GlobalData.User.ID == int.Parse(grid.CurrentRow.Cells["gridUserID"].Value.ToString()))
                            {
                                ...
                                //dal.Delete(arrear);
                                GlobalData.ProcessDelete(this.Name, arrear.ID);
