...
namespace eMAS.Forms.SystemSetup
{
    public partial class UserNew : Form
    {
        ...
        private void UserToRole(int type)
        {
            var roleId = GlobalData._context.Roles.SingleOrDefault(u => u.RoleName == cboRole.Text).RoleID;
            
            if (type == 1) //save
            {
                var userToRole = new DataReference.UsersToRole
                {
                    FKUserID = userID,
                    FKRoleID = roleId
                };
                GlobalData._context.UsersToRoles.InsertOnSubmit(userToRole);
            }
            else if(type == 2) //update
            {
                var currentRole = GlobalData._context.UsersToRoles.SingleOrDefault(u => u.FKUserID == userID);
        ...
        public void EditUser(User user)
        {
            try
            {
                ...
                cboRole_DropDown(this, EventArgs.Empty); 
                cboRole.Text = user.UserRole;
        ...
        private void viewbtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                view.btnDelete.Enabled = CanDelete;
                view.Show();
            }
            catch (Exception ex)
            {
        ...
        private void cboRole_DropDown(object sender, EventArgs e)
        {
            cboRole.Items.Clear();
            var getRoles = GlobalData._context.Roles.ToList();
            foreach (var roles in getRoles)
            {
                cboRole.Items.Add(roles.RoleName);
            }
        ...
        private void btnResetPassword_Click(object sender, EventArgs e)
        {
            try
            {
                if (GlobalData.User.UserName.Trim().ToLower() == "admin")
                {
                    var user = GlobalData._context.Users.Single(u => u.Archived == false && u.StaffID == txtStaffID.Text && u.UserName == userNameTextBox.Text);
                    ...
                    GlobalData._context.SubmitChanges();
                    Clear();
                    MessageBox.Show("Your password has be reseted to your username, you can change it from Exit->Change Password after you login");
                }
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
                MessageBox.Show("There was an error, kindly contact the admin");
                //throw ex;
            }
        }
    }
