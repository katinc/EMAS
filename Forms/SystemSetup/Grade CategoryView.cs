...
namespace eMAS.All_UIs.System_SetupFORMS
{
    public partial class Grade_CategoryView : Form, IRefreshView 
    {
        ...
        public void RefreshView()
        {
            try
            {
                ...
                foreach (GradeCategory gradeCategory in gradeCategories)
                {
                    ...
                    grid.Rows[ctr].Cells["gridOverTimeRate"].Value = gradeCategory.OverTimeRate;
                    grid.Rows[ctr].Cells["gridLeaveEncashmentRate"].Value = gradeCategory.LeaveEncashmentRate;
                    ctr++;
                }
        ...
        {
            var getPermissions = GlobalData._context.UserAccessLevelsViews.SingleOrDefault(u => u.AccessLevel2Control == this.Parent.Name && u.RoleName == GlobalData.Role);
