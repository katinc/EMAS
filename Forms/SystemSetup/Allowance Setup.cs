    ...
    public partial class Allowance_Setup : Form
    {
        ...
        private void levelsComboBox_DropDown(object sender, EventArgs e)
        {
            try
            {
                ...
                var dtEmpGrades = dalHelper.ExecuteReader(";WITH ue(id) AS (SELECT MAX(id) FROM [EmployeeGradeView] GROUP BY Description) SELECT * FROM [EmployeeGradeView] t INNER JOIN ue ON ue.id = t.id where t.Archived='false'");

                foreach (DataRow dRow in dtEmpGrades.Rows)
                {
                    var grade = new EmployeeGrade();
                    grade.ID = (!Information.IsDBNull(dRow["ID"])) ? int.Parse(dRow["ID"].ToString()) : 0;
                    grade.Grade = (!Information.IsDBNull(dRow["Description"])) ? dRow["Description"].ToString() : null;
                   /* grade.GradeCategory.ID = int.Parse(dRow["GradeCategoryID"].ToString());
                    grade.Level = dRow["GradeLevel"].ToString();
                    grade.StartStep = (!Information.IsDBNull(dRow["StartStep"])) ? int.Parse(dRow["StartStep"].ToString()) : 0;
                    grade.User.ID = (!Information.IsDBNull(dRow["UserID"])) ? int.Parse(dRow["UserID"].ToString()) : 0;*/
        ...
        private void levelsComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (Allowance allowance in allowances)
                {
                    if (allowance.Level.Grade == grades[levelsComboBox.SelectedIndex].Grade && allowance.Archived==false)
                    {
        ...
        private void viewbtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                allowanceSetupView.btnRemove.Enabled = CanDelete;
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (grid.SelectedRows.Count > 0)
                {
                    ...
                    {
                        dalHelper.CreateParameter("AID", int.Parse(grid.CurrentRow.Cells["gridID"].Value.ToString()), DbType.Int32);
                        dalHelper.ExecuteNonQuery("update  Allowance_Setup set Archived='true' where AID=@AID");
