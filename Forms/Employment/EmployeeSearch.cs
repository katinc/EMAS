...
namespace eMAS.All_UIs.Staff_Information_FORMS
{
    public partial class Employee_Search : Form,IRefreshView
    {
        ...
        public Employee_Search(IDAL dal, EmployeeMaintenanceHTH viewForm)
        {
            try
            {
                InitializeComponent();
        ...
        private void gridSearchResults_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                //if (employeeRegistration.getPermissions.CanEdit || employeeRegistrationHTH.)
                //{
                    if (grid.CurrentRow != null)
                    {
                        if (GlobalData._context.CompanyInfos.FirstOrDefault().Initials == "HTH")
                        {
                            employeeRegistrationHTH.PopulateView(grid.CurrentRow);
        ...
        private void GetData()
        {
            try
            {
                ...
                if (cboOccupationalGroup.Text.Trim() != string.Empty)
                {
                    query.Criteria.Add(new Criterion()
                    {
                        Property = "StaffPersonalInfoLazyLoadView.OccupationGroup",
                        CriterionOperator = CriterionOperator.EqualTo,
                        Value = occupationGroups[cboOccupationalGroup.SelectedIndex].Description,
                        CriteriaOperator = CriteriaOperator.And
                    });
                }
                ...
                if (cboDirectorate != null && cboDirectorate.Text != string.Empty)
                {
                    query.Criteria.Add(new Criterion()
                    {
                        Property = "StaffPersonalInfoLazyLoadView.Directorate",
                        CriterionOperator = CriterionOperator.EqualTo,
                        Value = cboDirectorate.Text,
                        CriteriaOperator = CriteriaOperator.And
                    });
                }
                if (cboSubBMC != null && cboSubBMC.Text != string.Empty)
                {
                    query.Criteria.Add(new Criterion()
                    {
                        Property = "StaffPersonalInfoLazyLoadView.SubBMC",
                        CriterionOperator = CriterionOperator.EqualTo,
                        Value = cboSubBMC.Text,
                        CriteriaOperator = CriteriaOperator.And
                    });
                }
        ...
        private void cboOccupationalGroup_DropDown(object sender, EventArgs e)
        {
            try
            {
                cboOccupationalGroup.Items.Clear();
                if (occupationGroups.Count <= 0)
                {
                    occupationGroups = dal.GetAll<OccupationGroup>();
                }

                foreach (OccupationGroup zone in occupationGroups)
                {
                    cboOccupationalGroup.Items.Add(zone.Description);
                }
            }
            catch (Exception ex)
            {
        ...
        private void cboDirectorate_DropDown(object sender, EventArgs e)
        {
            try
            {
                cboDirectorate.Items.Clear();

                var directorates = GlobalData._context.Directorates.Where(u => u.Active).ToList();

                foreach (var item in directorates)
                {
                    cboDirectorate.Items.Add(item.Description);
                }
            }
            catch (Exception ex)
            {
        ...
        private void cboSubBMC_DropDown(object sender, EventArgs e)
        {
            try
            {
                ...
                var subs = GlobalData._context.SubBMCs.Where(u => u.Active).ToList();
