...
namespace eMAS.All_UIs.Staff_Information_FORMS
{
    ...
    delegate void FunctionHTH();	// a simple delegate for marshalling calls from event handlers to the GUI thread

    public partial class EmployeeMaintenanceHTH : Form
    {
        ...
        //Personal Detail
        IList<Zone> zones;
        IList<Directorate> directorates;
        ...
        DataTable dtAnnualLeave;

      
        public EmployeeMaintenanceHTH()
        {
            try
            {
                InitializeComponent();
                ...
                DOFADatePicker.Value = DateTime.Today;
                DOCADatePicker.Value = DateTime.Today;
                startDatePicker.Value = DateTime.Today;
                assumptionDatePicker.Value = DateTime.Today;
                endDatePicker.Value = DateTime.Today;
                dateEmploymentDate.Value = DateTime.Today;
                dateEngagementEffectiveDate.Value = DateTime.Today;
                dateEngagementDate.Value = DateTime.Today;
                dateEngagementEndingDate.Value = DateTime.Today;
        ...
        void cboBirthDistrict_DropDown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    //staffErrorProvider.SetError(cboBirthDistrict, "Please select Birth Region first");
                    return;
                }
                else
                {
                    
                        var birthDistricts = GlobalData._context.DistrictViews.Where(u => u.Region == cboBirthRegion.Text.Trim()).ToList();

                        foreach (var districts in birthDistricts)
                        {
                            cboBirthDistrict.Items.Add(districts.Description);
                        }
                    
                    //var regionID = GlobalData._context.Regions.FirstOrDefault(u => u.Description.Contains(cboBirthRegion.Text));
        ...
        void cmbHomeTown_DropDown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var hometowns = GlobalData._context.Towns.OrderBy(u => u.Description).ToList();
                foreach (var towns in hometowns)
                {
                    cmbHomeTown.Items.Add(towns.Description);
                }
        ...
        {
            if (gradeCategoryComboBox.Items.Count >0 )
            {
                return ;
            }
            try
        ...
        void gridExperience_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (gridEmploymentHistory.CurrentCell.ColumnIndex == 3)
                {
        ...
        public void QuickSearchView(string search)
        {
            try
            {
                ...
                //since it's loaded from the quick view, you cant allow editing 
                btnSave.Enabled = permissions.CanEdit;
                ...
                if (employee.StaffID != null && employee.StaffID != string.Empty)
                {
                    ...
                    txtGPS.Text = employee.GPSAddress.ToString();
                    txtVehicleDescription.Text = employee.VehicleDescription;
                    txtVehicleNumber.Text = employee.VehicleNumber;
                    cboVehicleType_DropDown(this, EventArgs.Empty);
                    cboVehicleType.Text = employee.VehicleType;

                    cboSubBMC.Text = employee.SubBMC;
                    ...
                    txtFileNumber.Text = employee.FileNumber;
                    txtFileLocation.Text = employee.FileLocation;

                    txtTribe_DropDown(this, EventArgs.Empty);
                    ...
                    cmbDirectorate_DropDown(this, EventArgs.Empty);
                    cmbDirectorate.Text = employee.directorates.Description; 
                    ...
                    {
                        conversionDatePicker.Checked = false;
                    }
                    else
                    {
                        conversionDatePicker.Value = DateTime.Parse(employee.ConversionDate.ToString()).Date;
                        conversionDatePicker.Checked = true;
                    }

                    if (employee.UpgradeDate == null)
                    {
                        upgradeDatePicker.Checked = false;
                    }
                    else
                    {
                        upgradeDatePicker.Value = DateTime.Parse(employee.UpgradeDate.ToString()).Date;
                        upgradeDatePicker.Checked = true;
                    }
                    ...
                    cboLeaveEntitlement.Text = employee.AnnualLeaveEntitlement.Status + " (" + employee.AnnualLeaveEntitlement.NumberOfDays + ")";
                    ...
                    //addition
                    //paymentacctype_DropDown(this, EventArgs.Empty);
                    paymentacctype.Text = employee.PaymentAccType;
                    //end addtion
        ...
        public void PopulateView(DataGridViewRow row)
        {
            try
            {
                ...
                if (employee.StaffID != null && employee.StaffID != string.Empty)
                {
                    ...
                    //cboZone.DropDownStyle = ComboBoxStyle.DropDown;
                    cboZone_DropDown(this, EventArgs.Empty);
                    cboZone.Text = employee.Zone.Description;
                   //cboZone.DropDownStyle = ComboBoxStyle.DropDownList;
                    ...
                    //cmbHomeRegion_DropDown(this, EventArgs.Empty);
                    //cmbHomeRegion.Text = employee.HomeTown.District.Region.Description;
                    ...
                    cboVehicleType_DropDown(this, EventArgs.Empty);
                    cboVehicleType.Text = employee.VehicleType;
                    cboSubBMC_DropDown(this, EventArgs.Empty);
                    cboSubBMC.Text = employee.SubBMC;
        ...
        private void viewButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                searchForm.btnRemove.Enabled = permissions.CanDelete;
                searchForm.Show();
                searchForm.Focus();
        ...
        {         
            try
            {
                ...
                tabOtherDetails.TabPages.Remove(UserAccountTabPage);
                var str = GlobalData.ItemControl;

                permissions = GlobalData.CheckPermissions(2);
                btnSave.Enabled = permissions.CanAdd;
                viewButton.Enabled = permissions.CanView;
                btnQuickSearch.Enabled = permissions.CanView;
                btnRemoveEmployee.Enabled = permissions.CanDelete;
                ...
                if (facility.Count() > 0)
                {
                    foreach (var item in facility)
                    {
                        ...
                        {
                            label37.Text = "Source of Fund:";
                            //change location to compensate for the change in characters
                            //left for horizontal
                            label37.Left += 32;
                        }
                        else if (item.Facility.ToString().Equals("HTH"))
                        {
                            lblPF.Visible = false;
                            numericPF.Visible = false;
                            checkBoxExemptFromSecondTier.Visible = false;
                            dateEngagementEffectiveDate.Visible = false;
                            label91.Visible = false;
                            dateEmploymentDate.Enabled = true;
                            label37.Visible = false;
                            paymentacctype.Visible = false;
                            withholdingTaxCheckBox.Visible = false;
                            incomeCheckBox.Visible = true;
                            isProvidentFundCheckBox.Visible = false;
        ...
        private string GenerateStaffID()
        {
            ...
            try
            {
                ...
                object result = null;
                result = dalHelper.ExecuteScalar("Select Max(StaffID) From StaffPersonalInfo ");
                if (result == null || result.ToString().Trim() == string.Empty)
                {
                    staffID = this.company.Character + string.Empty+this.company.InitialStaffID;
                }
                else
                {
                    try
                    {
                        ...
                        if(result.ToString().Trim().Length==numberPart.Trim().Length)
                        {
                            if(!Information.IsNumeric(numberPart.Substring(0,2)))
                                numberPart = result.ToString().Substring(2, result.ToString().Length - 2);
        ...
        {             
            try
            {
                ...
                if (ValidateBasic()
                    ...
                    //ValidateRelations() && 
                    //ValidateLanguages() && ValidateProffessionHistory() && ValidateEmploymentHistory() && ValidateEducationHistory() && ValidateDocuments() && ValidateReferees() && ValidateWorkPermit() && ValidatePassport() && ValidateVisa()
                    )
                {
                    ...
                    if (conversionDatePicker.Checked)
                    {
                        dalHelper.CreateParameter("@ConversionDate", conversionDatePicker.Value, DbType.Date);
                    }
                    else
                    {
                            dalHelper.CreateParameter("@ConversionDate", DBNull.Value, DbType.Date);
                        
                    }

                    if (upgradeDatePicker.Checked)
                    {
                        dalHelper.CreateParameter("@UpgradeDate", upgradeDatePicker.Value, DbType.Date);
                    }
                    else
                    {
                        dalHelper.CreateParameter("@UpgradeDate", DBNull.Value, DbType.Date);

                    }

                    if (cboRace.Text.Trim() != string.Empty && cboRace.SelectedIndex>-1)
                    {
                    ...
                    if (cboLeaveEntitlement.Text.Trim() != string.Empty && cboLeaveEntitlement.SelectedIndex>-1)
                    {
                    ...
                    if (cboLicenceType.Text.Trim() != string.Empty && cboLicenceType.SelectedIndex>-1)
                    {
                    ...
                    if (cmbPOB.Text.Trim() != string.Empty && cmbPOB.SelectedIndex>-1)
                    {
                    ...
                    if (cboBirthCountry.Text.Trim() != string.Empty && cboBirthCountry.SelectedIndex>-1)
                    {
                    ...
                    if (cboBirthDistrict.Text.Trim() != string.Empty && cboBirthDistrict.SelectedIndex>-1)
                    {
                        var birthDistricts = GlobalData._context.DistrictViews.First(u => u.Description == cboBirthDistrict.Text);
                        dalHelper.CreateParameter("@DistrictOfBirth", birthDistricts.ID, DbType.Int32);
                    }
                    else
                    {
                        dalHelper.CreateParameter("@DistrictOfBirth", 0, DbType.Int32);
                    }
                    if (cboBirthRegion.Text.Trim() != string.Empty && cboBirthRegion.SelectedIndex>-1)
                    {
                    ...
                    if (cboDenomination.Text.Trim() != string.Empty && cboDenomination.SelectedIndex>-1)
                    {
                    ...
                    if (cmbTitle.Text.Trim() != string.Empty && cmbTitle.SelectedIndex>-1)
                    {
                    ...
                    if (cmbHomeTown.Text.Trim() != string.Empty && cmbHomeTown.SelectedIndex>-1)
                    {
                        var towns = GlobalData._context.TownViews.First(u => u.Description == cmbHomeTown.Text);
                        dalHelper.CreateParameter("@HomeTown", towns.ID, DbType.Int32);
                    ...
                    if (cboNationality.Text.Trim() != string.Empty && cboNationality.SelectedIndex>-1)
                    {
                    ...
                    if (cboZone.Text.Trim() != string.Empty && cboZone.SelectedIndex>-1)
                    {
                    ...
                    if (cmbDirectorate.Text.Trim() != string.Empty && cmbDirectorate.SelectedIndex>-1)
                    {
                        dalHelper.CreateParameter("@DirectorateID", directorates[cmbDirectorate.SelectedIndex].ID, DbType.Int32);
                    }
                    else
                    {
                        dalHelper.CreateParameter("@DirectorateID", 0, DbType.Int32);
                    }
                    ...
                    if (cboContactCity.Text.Trim() != string.Empty && cboContactCity.SelectedIndex>-1)
                    {
                    ...
                    if (cboContactRegion.Text.Trim() != string.Empty && cboContactRegion.SelectedIndex>-1)
                    {
                    ...
                    if (cboContactCountry.Text.Trim() != string.Empty && cboContactCountry.SelectedIndex>-1)
                    {
                    ...
                    if (cboContactHomeTown.Text.Trim() != string.Empty && cboContactHomeTown.SelectedIndex>-1)
                    {
                    ...
                    if (cboResidentialCity.Text.Trim() != string.Empty && cboResidentialCity.SelectedIndex>-1)
                    {
                    ...
                    if (cboResidentialRegion.Text.Trim() != string.Empty && cboResidentialRegion.SelectedIndex>-1)
                    {
                    ...
                    if (cboResidentialCountry.Text.Trim() != string.Empty && cboResidentialCountry.SelectedIndex > -1)
                    {
                    ...
                    dalHelper.CreateParameter("@GPSAddress", txtGPS.Text.Trim(), DbType.String);
                                   
                    
                    //Job Detail
                    if (departmentComboBox.Text.Trim() != string.Empty && departmentComboBox.SelectedIndex>-1)
                    {
                    ...
                    if (cboSpecialty.Text.Trim() != string.Empty && cboSpecialty.SelectedIndex>-1)
                    {
                    ...
                    if (cboUnit.Text.Trim() != string.Empty && cboUnit.SelectedIndex>-1)
                    {
                    ...
                    if (cboOccupationGrp.Text.Trim() != string.Empty && cboOccupationGrp.SelectedIndex>-1)
                    {
                    ...
                    if (cboEmploymentStatus.Text.Trim() != string.Empty && cboEmploymentStatus.SelectedIndex>-1)
                    {
                    ...
                    if (probationCheckBox.Checked && cboProbationStatus.Text.Trim() != string.Empty && cboProbationStatus.SelectedIndex>-1)
                    {
                    ...
                    if (cboAppointmentType.Text.Trim() != string.Empty && cboAppointmentType.SelectedIndex > -1)
                    {
                    ...
                    if (cboJobTitle.Text.Trim() != string.Empty && cboJobTitle.SelectedIndex>-1)
                    {
                    ...
                    ////Engagement
                    if (cboEngagementType.Text.Trim() == string.Empty || cboAppointmentType.Text.ToLower().Trim() != "contract" || cboEngagementType.SelectedIndex == -1)
                    {
                    ...
                    if (cboEngagementGradeOn.Text.Trim() == string.Empty || cboAppointmentType.Text.ToLower().Trim() != "contract" || cboEngagementType.SelectedIndex == -1)
                    {
                    ...
                    if (dateEngagementDate.Value == null || dateEngagementDate.Checked == false  || cboEngagementType.SelectedIndex == -1)
                    {
                    ...
                    if (cboEngagementGradeLeaving.Text.Trim() == string.Empty || cboAppointmentType.Text.ToLower().Trim() != "contract" || cboEngagementType.SelectedIndex == -1) 
                    {
                    ...
                    if (dateEngagementEndingDate.Value == null || dateEngagementEndingDate.Checked == false  || cboEngagementType.SelectedIndex == -1)
                    {
                    ...
                    if (checkedListBoxContract.CheckedItems.Count <= 0 || cboEngagementType.SelectedIndex == -1)
                    {
                    ...
                    //Salary Detail
                    //new addition : payment acc type
                    if (paymentacctype.Text.Trim() != string.Empty && gradeCategoryComboBox.SelectedIndex > -1)
                    {
                        dalHelper.CreateParameter("@PaymentAccType", paymentacctype.Text, DbType.String);
                    }
                    else
                    {
                        dalHelper.CreateParameter("@PaymentAccType", DBNull.Value, DbType.String);

                    }//addition ends
                    if (gradeCategoryComboBox.Text.Trim() != string.Empty && gradeCategoryComboBox.SelectedIndex>-1)
                    {
                    ...
                    if (cboStep.Text.Trim() != string.Empty && cboStep.SelectedIndex>-1)
                    {
                    ...
                    {
                        dalHelper.CreateParameter("@BandID", SalLevelID, DbType.Int32);
                    }
                    else
                    {
                    ...
                    if (paymentTypeComboBox.Text.Trim() == string.Empty)
                    {
                        ...
                        //return;
                        dalHelper.CreateParameter("@PaymentType", 0, DbType.String);
                    }
                    else
                    {
                    ...
                    dalHelper.CreateParameter("@UserID", GlobalData.User.ID, DbType.Int32);
                    dalHelper.CreateParameter("@UserType", "EMPLOYEE", DbType.String);
                    dalHelper.CreateParameter("@VehicleNumber", txtVehicleNumber.Text, DbType.String);
                    dalHelper.CreateParameter("@VehicleDescription", txtVehicleDescription.Text, DbType.String);
                    dalHelper.CreateParameter("@VehicleType", cboVehicleType.Text, DbType.String);
                    dalHelper.CreateParameter("@SubBMC", cboSubBMC.Text, DbType.String);
                    ...
                    if (txtFileNumber.Text.Trim() == string.Empty)
                    {
                        ...
                        dalHelper.CreateParameter("@FileLocation", string.Empty, DbType.String);
                    }
                    else
                    { 
                        dalHelper.CreateParameter("@FileLocation", txtFileLocation.Text, DbType.String);
                        dalHelper.CreateParameter("@FileNumber", txtFileNumber.Text, DbType.String);
                    }
                    ...
                    if (editMode)
                    {
                        ...
                        string query = "Update StaffPersonalInfo Set StaffID=@StaffID,FingerPrintID=@FingerPrintID,Surname=@Surname,Firstname=@Firstname,OtherName=@OtherName,NickName=@NickName,FileNumber=@FileNumber,FileLocation=@FileLocation,PIN=@PIN,NationalID=@NationalID,MaidenName=@MaidenName,";
                        ...
                        query += "CurrentPromotionDate=@DOFA,DOFA=@DOFA,DOCA=@DOCA,AssumptionDate=@AssumptionDate,SpecialtyID=@SpecialtyID,UnitID=@UnitID,JobTitleID=@JobTitleID,OccuGRP=@OccuGRP,RaceID=@RaceID,Tribe=@Tribe,Overseer=@Overseer,SupervisorCode=@SupervisorCode,StatusID=@StatusID,ZoneID=@ZoneID,DirectorateID=@DirectorateID,InZoneDate=@InZoneDate,";
                        ...
                        query += "TransferredOut=@TransferredOut,IsProvidentFund=@IsProvidentFund,QualificationID=@QualificationID,TransferredIn=@TransferredIn,PaymentAccType=@PaymentAccType,UserType=@UserType, GPSAddress=@GPSAddress, ConversionDate=@ConversionDate, UpgradeDate=@UpgradeDate, VehicleNumber=@VehicleNumber, VehicleDescription=@VehicleDescription, VehicleType=@VehicleType, SubBMC=@SubBMC " +
                            "Where ID = @ID";
                    ...
                    {
                        string query = "Insert Into StaffPersonalInfo(StaffID,FingerPrintID,Surname,Firstname,OtherName,NickName,FileNumber,FileLocation,PIN,NationalID,MaidenName,";
                        ...
                        query += "RaceID,Tribe,Overseer,SupervisorCode,StatusID,ZoneID,DirectorateID,InZoneDate,TransferredOut,IsProvidentFund,QualificationID,TransferredIn,PaymentAccType,UserType, GPSAddress, ConversionDate, UpgradeDate, VehicleNumber, VehicleDescription, VehicleType, SubBMC)";

                        query += "Values" +
                            "(@StaffID,@FingerPrintID,@Surname,@Firstname,@OtherName,@NickName,@FileNumber,@FileLocation,@PIN,@NationalID,@MaidenName,@SalaryGrouping,@GradeCategoryID,@GradeID,@GradeOnFirstAppointmentID,@GradeDate,@StepID,@BandID,@Salary,@DOB,@GenderGroupID,";
                    ...
                    //create a backup for the just added or updated info
                    BackupStaffPersonalInfo();
                    ...
                    if (probationCheckBox.Checked)
                    {
                        ...
                        //update history
                        dalHelper.ExecuteNonQuery("Update StaffPersonalInfoHistory Set ProbationStartDate=@ProbationStartDate,ProbationEndDate=@ProbationEndDate Where ID=@ID");
                    ...
                    if (allowedToSaveAll)
                    {
                        ...
                        SaveDocuments();
                        SaveReferee();
                        if (cboBankDetailName.SelectedIndex > -1 && cboBankDetailBranch.SelectedIndex > -1 && cboBankDetailAccountType.SelectedIndex > -1 && txtBankDetailAccountNumber.Text != string.Empty)
                        {
                            SaveStaffBanks();
                        }
                        SaveOthers();                      
            ...
            catch (Exception ex)
            {
                ...
                Logger.LogError(ex);
                Logger.WriteToLogFile(ex.StackTrace.ToString());
                MessageBox.Show("Could Not Save Successfully. Make sure all required fields are entered correctly!,Please See the System Administrator", "Error");
            }
        }
        ...
        private void SaveStaffBanks()
        {
            try
            {
                ...
                staffBank.AccountNumber = txtBankDetailAccountNumber.Text;
                staffBank.AccountType.ID = accountTypes[cboBankDetailAccountType.SelectedIndex].ID;
                //staffBank.BankBranch.ID = int.Parse(GlobalData._context.StaffBankViews.First(u => u.BankBranch == cboBankDetailBranch.Text).ID.ToString());
                //staffBank.BankBranch.ID = Convert.ToInt32(GlobalData._context.BankBranchViews.FirstOrDefault(u => u.BankID == staffBank.Bank.ID && u.Description.Equals(cboBankDetailBranch.Text )).id);
                staffBank.BankBranch.ID = bankBranches[cboBankDetailBranch.SelectedIndex].ID;
                staffBank.In_Use = true;
        ...
        private void BackupStaffPersonalInfo()
        {
            try
            {
                string query = "Insert Into StaffPersonalInfoHistory (StaffID,Surname,Firstname,OtherName,NickName,FileNumber,FileLocation,PIN,NationalID,MaidenName,";
                query += "SalaryGrouping,GradeCategoryID,GradeID,GradeOnFirstAppointmentID,GradeDate,StepID,BandID,BasicSalary,DOB,GenderGroupID,TitleID,NationalityID,TownID,Hometown,Telno,MobileNo,MaritalStatusID,DOM,ReligionID,";
                query += "Placeofbirth,CountryOfBirth,DistrictOfBirth,RegionOfBirth,NoOfChildren,Denomination,NHISNumber,HouseNumber,StreetName,ResidentialTownID,";
                query += "ResidentialRegionID,ResidentialCountryID,Address,Email,RegionID,ContactCountryID,ContactHomeTownID,DepartmentID,AppointmentTypeID,EmploymentDate,Mechanised,SSNIT,";
                query += "IncomeTax,Probation,ProbationStartDate,ProbationEndDate,ProbationStatusID,SSNITNo,TIN,Payment,AnnualLeaveEntitlementID,PaymentType,UserID,EngagementTypeID,EngagementGradeIDOn,";
                ...
                query += "IsDisable,DisabilityTypeID,DisabilityDate,LicenceTypeID,LicenceNumber,EngagementAnnualSalary,";
                query += "PFRate,IsExemptFromSecondTier,IsSusuPlusContribution,SusuPlusContributionAmount,IsWithholdingTax,IsWithholdingTaxFixedAmount,WithholdingTaxFixedAmount,IsWithholdingTaxRate,WithholdingTaxRate,CalculateOn,";
                query += "RaceID,Tribe,Overseer,SupervisorCode,StatusID,ZoneID,DirectorateID,InZoneDate,TransferredOut,IsProvidentFund,QualificationID,TransferredIn,PaymentAccType,UserType, GPSAddress)";
                query += "Values(@StaffID,@Surname,@Firstname,@OtherName,@NickName,@FileNumber,@FileLocation,@PIN,@NationalID,@MaidenName,@SalaryGrouping,@GradeCategoryID,@GradeID,@GradeOnFirstAppointmentID,@GradeDate,@StepID,@BandID,@Salary,@DOB,@GenderGroupID,";
                query += "@TitleID,@NationalityID,@ContactTownID,@HomeTown,@ContactTelephone,@ContactMobileNumber,@MaritalStatusID,@DOM,@ReligionID,";
                query += "@POB,@CountryOfBirth,@DistrictOfBirth,@RegionOfBirth,@NoOfChildren,@Denomination,@NHISNumber,@ResidentialHouseNumber,@ResidentialStreetName,@ResidentialTownID,";
                query += "@ResidentialRegionID,@ResidentialCountryID,@ContactPostalAddress,@ContactEmailAddress,@ContactRegionID,@ContactCountryID,@ContactHomeTownID,@DepartmentID,@AppointmentTypeID,";
                query += "@EmploymentDate,@Mechanised,@SSNIT,@IncomeTax,@Probation,@ProbationStartDate,@ProbationEndDate,@ProbationStatusID,@SSNITNo,@TIN,@Payment,@AnnualLeaveEntitlementID,@PaymentType,@UserID,";
        ...
        private bool ValidatePersonalInfo()
        {
            ...
            try
            {
                ...
                if (editMode == false)
                {
                    var checkName = GlobalData._context.StaffPersonalInfos.Where(u => u.Firstname == txtFirstName.Text && u.OtherName == txtOtherName.Text && u.Surname == surnameTextBox.Text).ToList();
                    if (checkName.Count() > 0)
                    {
                        result = false;
                        staffErrorProvider.SetError(txtFirstName, "Another Employee is Registered with the same name");
                        txtFirstName.Focus();
                    }
                }
                ...
                //}
                //else if (cmbMaritalStatus.SelectedItem.ToString() == "Married" && (dateDOM.Value == null))
                //{
                //    result = false;
                ...
                else if (txtContactEmailAddress.Text.Trim() != string.Empty)
                {
                    ...
                    result = new Regex(@"^\s*[\w\-\+_']+(\.[\w\-\+_']+)*\@[A-Za-z0-9]([\w\.-]*[A-Za-z0-9])?\.[A-Za-z][A-Za-z\.]*[A-Za-z]$", RegexOptions.IgnoreCase)
                    .IsMatch(txtContactEmailAddress.Text.Trim());
        ...
        private void ClearPersonalInfo()
        {
            try
            {
                ...
                conversionDatePicker.ResetText();
                upgradeDatePicker.ResetText();

                cboRace.Items.Clear();
                cboRace.Text = string.Empty;

                txtTribe.Items.Clear();
                ...
                cmbDirectorate.Items.Clear();
                cmbDirectorate.Text = string.Empty;
                cboQualificationType.Items.Clear();
                cboQualificationType.Text = string.Empty;
        ...
        private void btnDocumentsView_Click(object sender, EventArgs e)
        {
            try
            {
                if (gridDocuments.CurrentRow != null && gridDocuments.CurrentRow.Cells["gridDocumentsPath"].Value != null)
                {
                    ...
                    string extension = gridDocuments.CurrentRow.Cells["gridDocumentsDocumentType"].Value.ToString().ToLower();
                    ...
                    //else
                    //{
                    //    //this.Cursor.cu = Cursor.Current.
                    //    reader1 = new DocForm();
                    //    reader1.Show();
                    //    reader1.LoadDocument(gridDocuments.CurrentRow.Cells["gridDocumentsPath"].Value.ToString(), extension);
                                               
                    //}

                    byte[] bytes = (byte[])gridDocuments.CurrentRow.Cells["gridDocumentsDocumentsContent"].Value;
            ...
            try
            {
                if (employee.ID != 0)
                {
                    if(ValidateEmploymentHistory())
                    {
                        ...
                        {
                            Logger.LogError(ex);
                            MessageBox.Show("Could Not Save Successfully, Please See the System Administrator", "ERROR");
                        }
        ...
        private void ClearAll()
        {
            try
            {
                ClearPersonalInfo();
                //Details
                txtVehicleNumber.ResetText();
                cboVehicleType.Items.Clear();
                txtVehicleDescription.ResetText();
                cboSubBMC.Items.Clear();
                gridRelations.Rows.Clear();
                gridLanguage.Rows.Clear();
                gridProfession.Rows.Clear();
                gridEmploymentHistory.Rows.Clear();
        ...
        private void cboContactHomeTown_DropDown(object sender, EventArgs e)
        {
            ...
            //    dalHelper.CreateParameter("@Archived", "False", DbType.String);
            //    dalHelper.CreateParameter("@Active", "True", DbType.String);
            //    //dalHelper.CreateParameter("@Region", cboContactRegion.Text, DbType.String);
            //    contactHomeTownsTable = dalHelper.ExecuteReader("select ID,Description,Region From ViewTowns  WHERE Archived=@Archived and Active=@Active ORDER BY Description ASC");
              
            //    foreach (DataRow row in contactHomeTownsTable.Rows)
            //    {
            //       //if(cmbHomeRegion.Text.Trim().ToLower()==row["Region"].ToString().Trim().ToLower())
        ...
        private void cboBirthDistrict_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                ...
                cmbPOB.Items.Clear();
                dalHelper.ClearParameters();
                //added to get selected district
                dalHelper.CreateParameter("@Description", cboBirthDistrict.SelectedItem, DbType.String);

                dalHelper.CreateParameter("@Archived", "False", DbType.String);
                dalHelper.CreateParameter("@Active", "True", DbType.String);
                //changes sql query PREVIOUS : "select ID as ID,Description,District  From ViewTowns WHERE Archived=@Archived and Active=@Active order BY Description ASC"
                birthPlacesTable = dalHelper.ExecuteReader("select TownView.ID as ID,TownView.Description as Description From TownView WHERE " +
                        "TownView.Archived = @Archived and TownView.Active = @Active and TownView.District = @Description order BY TownView.Description ASC");
                townsTable = birthPlacesTable;
                //var newTable = new DataTable();

                foreach (DataRow row in birthPlacesTable.Rows)
                {
                    cmbHomeTown.Items.Add(row["Description"].ToString());
                    // if (row["Description"].ToString().ToLower().Trim() == cboBirthDistrict.Text.ToLower().Trim())
                    // {
                    //newTable.Rows.Add(row["Description"]);
                ...
                //birthPlacesTable =new DataTable();
                //birthPlacesTable = newTable;
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
        ...
        private void cboContactRegion_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                ...
                dalHelper.ClearParameters();
                dalHelper.CreateParameter("@Region", cboContactRegion.SelectedItem, DbType.String);
                dalHelper.CreateParameter("@Archived", false, DbType.Boolean);
                dalHelper.CreateParameter("@Active", true, DbType.Boolean);
                contactTownsTable = dalHelper.ExecuteReader("select TownView.ID as ID,TownView.Description as Description,Region From TownView where Region=@Region and TownView.Archived=@Archived and TownView.Active=@Active  order BY TownView.Description ASC");
        ...
        private void cboBankDetailName_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                ...
                query.Criteria.Add(new Criterion()
                {
                    Property = "BankBranchView.BankName",
                    CriterionOperator = CriterionOperator.EqualTo,
        ...
        private void cboEngagementGradeOn_DropDown(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (!cboEngagementGradeOn.Items.Contains(row["Description"].ToString()))
        ...
        {
            //This is the old Logic. I will comment this and implement it with the new logic and see how it goes
            ...
            /*try
            {
                ...
                {*/
                    ...
                    */
                    //will have to uncomment later below lines 
        ...
        private void cboGradeOnFirstAppointment_DropDown(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (!cboGradeOnFirstAppointment.Items.Contains(row["Description"].ToString()))
        ...
        //Pull my leave entitlements
        void getLeaveEntitlements()
        {
            try
            {
                cboLeaveEntitlement.Items.Clear();
                
                if (dtAnnualLeave == null || dtAnnualLeave.Rows.Count < 1)
                {
                    dtAnnualLeave = new DataTable();
                    SqlDataAdapter adapta = new SqlDataAdapter("select * from AnnualLeaveEntitlement where active='true' and archived='false'", dalHelper.ConnectionString());
                    //adapta.SelectCommand.Parameters.AddWithValue("UserID", GlobalData.User.ID);
                    //adapta.SelectCommand.Parameters.AddWithValue("typesOfGrade", gradeComboBox.SelectedItem.ToString().Trim());

                    adapta.Fill(dtAnnualLeave);
                }
                  
                annualLeaveEntitlements=new List<AnnualLeaveEntitlement>();
                int index = 0,selectedIndex=-1;
                foreach (DataRow dataRow in dtAnnualLeave.Rows)
                {
                    var annualLeave = new AnnualLeaveEntitlement() {
                        ID =int.Parse( dataRow["ID"].ToString())
                    };

                        annualLeaveEntitlements.Add(annualLeave);
                    var Item = dataRow["Status"].ToString() + " (" + dataRow["NumberOfDays"].ToString() + ")";
                    //var Item = dataRow["CategoryOfPost"].ToString() + " " + dataRow["Status"].ToString() + " (" + dataRow["NumberOfDays"].ToString() + ")";

                    cboLeaveEntitlement.Items.Add(Item);

                    var gridCategory=dataRow["TypesOfGrade"].ToString().Trim().ToLower();
                    var gradesSArray = gridCategory.Split(new[] { ','});
                    var selectedCategory= gradeComboBox.SelectedItem == null ? string.Empty : gradeComboBox.SelectedItem.ToString().Trim().ToLower();

                    foreach (string el in gradesSArray)
                    {
                ...
                cboLeaveEntitlement.SelectedIndex = selectedIndex;
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
            ...
            try
            {
               ...
               /* if (employee.Unit.Description.Trim() != string.Empty && employee.Unit.Description.ToLower().Trim() != "not applicable" && employee.Confirmed == true)
                ...
                if (employee.AssumptionDate != null && employee.Confirmed == true)
                {
                    ...
                    assumptionDatePicker.Enabled = true;
                }
        ...
        private void getStaffFingerPrints()
        {
            gridFingerprints.Rows.Clear();

            staffFingerprintTemplates = mineObj.getStaffFingerprints(employee.StaffID);
        ...
        {
            if (string.IsNullOrEmpty(employee.StaffID) || string.IsNullOrEmpty(employee.FirstName))
            {
                MessageBox.Show("Sorry! You can only enroll already existing employees.\nPlease save details first");
            }
            else
            {
                EnrollFingerTemplate tmp = new EnrollFingerTemplate(employee.ID,employee.StaffID,"STAFF",string.Format("{0} {1} {2}", employee.FirstName, employee.Surname, employee.OtherName), getAlreadyEnrolledFingers());
        ...
        private void txtTribe_DropDown(object sender, EventArgs e)
        {
            try
            {
                dalHelper.ClearParameters();
                //dalHelper.CreateParameter("@Active", "True", DbType.String);
                DataTable tbl = dalHelper.ExecuteReader("select distinct Tribe from Tribes order by Tribe asc");
                txtTribe.Items.Clear();
                foreach (DataRow row in tbl.Rows)
                {
                    txtTribe.Items.Add(row["Tribe"].ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        {
            gradeComboBox_SelectedIndexChanged(sender, e);
        }

        private int getSalLevelID(string level)
        {
            dalHelper.ClearParameters();
            dalHelper.CreateParameter("@Archived", false, DbType.Boolean);
            dalHelper.CreateParameter("@Description", level, DbType.String);
            bandsTable = dalHelper.ExecuteReader("Select top 1 * from BandView Where BandView.Archived = @Archived and BandView.Description=@Description ");
            if (bandsTable.Rows.Count > 0)
            {
                return Convert.ToInt32( bandsTable.Rows[0]["id"]);
            }
            return 0;
        ...
        private void gradeComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            dalHelper.ClearParameters();
            dalHelper.CreateParameter("@GradeCategory", gradeCategoryComboBox.SelectedItem, DbType.String);
            dalHelper.CreateParameter("@Description", gradeComboBox.SelectedItem, DbType.String);
            DataTable tbl = dalHelper.ExecuteReader("Select e.*,s1.description as start_desc,s1.numeric_part as start_numeric_part,s2.numeric_part as end_numeric_part,s2.description as end_desc from EmployeeGradeView e left outer join ViewSortedStepsAlphaNumeric s1 on  s1.id=e.startstep left outer join ViewSortedStepsAlphaNumeric s2 on s2.id=e.endstep Where e.GradeCategory = @GradeCategory and e.Description=@Description  order by e.Description asc");
            cboBand.Items.Clear();
            cboStep.Items.Clear();
            ...
            if (null == stepsTable)
            {
                ...
                stepsTable.Columns.Add("ID");
                stepsTable.Columns.Add("STEP");
            }
            else
            {
                stepsTable.Rows.Clear();
            }
        

            foreach (DataRow row in tbl.Rows)
            {
                cboBand.Items.Add(row["GradeLevel"].ToString());
                if (null != row["start_numeric_part"] && !string.IsNullOrEmpty(row["start_numeric_part"].ToString()))
                {
                    int sstep, estep;
                    sstep = Convert.ToInt32(row["start_numeric_part"]);
                    estep = Convert.ToInt32(row["end_numeric_part"]);


                    dalHelper = new DALHelper();
                    dalHelper.Parameters.Clear();

                    dalHelper.CreateParameter("@start_step", sstep, DbType.Int32);
                    dalHelper.CreateParameter("@end_step", estep, DbType.Int32);
                    ...
                    if (dtGradeSteps.Rows.Count>0)
                    {
                        //int ctr=0;
                        foreach(DataRow rowStep in dtGradeSteps.Rows)
                        {
                            
                            DataRow newRow = stepsTable.NewRow();

                            newRow["ID"] = rowStep["id"];
                            newRow["STEP"] = rowStep["description"];

                            stepsTable.Rows.Add(newRow);

                            cboStep.Items.Add(rowStep["numeric_part"]);
                           // ctr++;
                        }
                    }
                }
            ...
            if(cboBand.Items.Count > 0)  cboBand.SelectedIndex = 0;
            if(cboStep.Items .Count > 0) cboStep.SelectedIndex = 0;
            setSalaryValue();
            SalLevelID = getSalLevelID(cboBand.Text);
        ...
        {
            if (cboStep.SelectedIndex >= 0 && gradeComboBox.SelectedIndex >= 0 && gradeCategoryComboBox.SelectedIndex >= 0)
            {
                DataRow r = getStepSalary(gradeCategories[gradeCategoryComboBox.SelectedIndex].ID, Convert.ToInt32(gradesTable.Rows[gradeComboBox.SelectedIndex]["ID"]), Convert.ToInt32(stepsTable.Rows[cboStep.SelectedIndex]["ID"]));
                if (r != null)
                {
                    txtSalary.Text = r["BasicSalary"].ToString();
        ...
        private void cmbDirectorate_DropDown(object sender, EventArgs e)
        {
            try
            {
                dalHelper.ClearParameters();
                directorates  = dal.GetAll<Directorate>();
                cmbDirectorate.Items.Clear();
                cmbDirectorate.Text = string.Empty;
                foreach (Directorate d in directorates)
                {
                    cmbDirectorate.Items.Add(d.Description);
                }
            }
            catch (Exception ex)
            {
        ...
        private void cboLeaveEntitlement_SelectedIndexChanged(object sender, EventArgs e)
        {

        }

        private void cmbJobTitle_DropDown(object sender, EventArgs e)
        {
            
        }

        private void DOFADatePicker_ValueChanged(object sender, EventArgs e)
        {
            try
            {
                dateEmploymentDate.Value = DOFADatePicker.Value;

            }
            catch (Exception xi) { }
        }

        //private void cmbHomeRegion_DropDown(object sender, EventArgs e)
        //{
        //    try
        //    {
        //        dalHelper.ClearParameters();
        ...
        //        dalHelper.CreateParameter("@Active", "True", DbType.String);
        //        contactRegionsTable = dalHelper.ExecuteReader("select RegionView.ID,RegionView.Description from RegionView where RegionView.Archived=@Archived and RegionView.Active=@Active order BY RegionView.Description ASC");
        //        cmbHomeRegion.Items.Clear();
        //        foreach (DataRow row in contactRegionsTable.Rows)
        //        {
        //            cmbHomeRegion.Items.Add(row["Description"].ToString());
        //        }
        //    }
        //    catch (Exception ex)
        //    {
        ...
        private void cboContactCity_DropDown(object sender, EventArgs e)
        {

        }

        private void cboBankDetailBranch_SelectedIndexChanged(object sender, EventArgs e)
        {

        }

        private void txtResidentialHouseNumber_TextChanged(object sender, EventArgs e)
        {
        ...
        private void cboVehicleType_DropDown(object sender, EventArgs e)
        {
            try
            {
                cboVehicleType.Items.Clear();

                var vehicleType = GlobalData._context.VehicleTypes.Where(u => u.Active).OrderBy(u => u.Description);

                foreach (var item in vehicleType)
                {
                    cboVehicleType.Items.Add(item.Description);
                }
            }
            catch (Exception ex)
            {
        ...
        private void cboSubBMC_DropDown(object sender, EventArgs e)
        {
            try
            {
                cboSubBMC.Items.Clear();

                var subBMC = GlobalData._context.SubBMCs.Where(u => u.Active).ToList();

                foreach (var item in subBMC)
                {
                    cboSubBMC.Items.Add(item.Description);
                }
            }
            catch (Exception ex)
            {
