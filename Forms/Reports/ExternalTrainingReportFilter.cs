...
{
    public partial class ExternalTrainingReportFilter : Form
    {
        ...
        public ExternalTrainingReportFilter()
        {
            try
            {
                InitializeComponent();
        ...
        private void cmbOption_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (cmbOption.Text == "All Employees")
                {
                    ...
                    cboSponsor.Visible = true;
                    lblRecommended.Visible = true;
                    cboRecommended.Visible = true;
                    lblApproved.Visible = true;
                    cboApproved.Visible = true;

                    lblSponsor.Visible = true;
                    lblRequestType.Visible = true;
                    lblQualification.Visible = true;
                    IblInstitution.Visible = true;

                    cboSponsor.Visible = true;
                    cboRequestType.Visible = true;
                ...
                else
                {
                    ...
                    cboSponsor.Visible = false;
                    lblRecommended.Visible = false;
                    cboRecommended.Visible = false;
                    lblApproved.Visible = false;
                    cboApproved.Visible = false;


                    lblSponsor.Visible = false;
                    lblRequestType.Visible = false;
                    lblQualification.Visible = false;
                    IblInstitution.Visible = false;

                    cboSponsor.Visible = false;
                    cboRequestType.Visible = false;
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string sql = "select * from ViewExternalTrainings ";
                DataTable dt;
                ...
                reportFilterDataRow["filterApproved"] = "All";
                reportFilterDataRow["filterRecommended"] = "All";

                reportFilterDataRow["filterSponsor"] = "All";
                reportFilterDataRow["filterInstitution"] = "All";
                reportFilterDataRow["filterJustified"] = "All";
                ...
                else
                {
                    ...
                    if (cboSponsor.SelectedItem != null)
                    {
                        dalHelper.CreateParameter("@Sponsor", cboSponsor.SelectedItem, DbType.String);
                        reportFilterDataRow["filterSponsor"] = cboSponsor.SelectedItem;
                        where += " Sponsor=@Sponsor and ";
                    }

                    if (cmbInstitution.SelectedItem != null)
                    {
                        dalHelper.CreateParameter("@School", cmbInstitution.SelectedItem, DbType.String);
                        reportFilterDataRow["filterInstitution"] = cmbInstitution.SelectedItem;
                ...
                if (errorText != string.Empty)
                    MessageBox.Show(errorText);
                        ...
                        if(cboApproved.Text=="Yes")
                        {
                            dalHelper.CreateParameter("@CEOApproved", true, DbType.Boolean);
                            reportFilterDataRow["filterApproved"]=cboApproved.SelectedItem;
                            where += " CEOApproved=@CEOApproved and ";
                        }
                        else if(cboApproved.Text=="No"){
                         dalHelper.CreateParameter("@CEOApproved",false,DbType.Boolean);
                            reportFilterDataRow["filterApproved"]=cboApproved.SelectedItem;
                            where += " CEOApproved=@CEOApproved and ";
                        }

                        
                        if (cboRecommended.Text == "Yes")
                        {
                            dalHelper.CreateParameter("@HRRecommended", true, DbType.Boolean);
                            reportFilterDataRow["filterRecommended"] = cboRecommended.SelectedItem;
                            where += " HRRecommended=@HRRecommended and ";
                        }
                        else if (cboRecommended.Text == "No")
                        {
                        ...
                        if (cmbJustified.Text == "Yes")
                        {
                            dalHelper.CreateParameter("@isJustified", true, DbType.Boolean);
                            reportFilterDataRow["filterJustified"] = cmbJustified.SelectedItem;
                            where += " isJustified=@isJustified and ";
                        }
                        else if (cmbJustified.Text == "No")
                        {
                        ...
                        {
                            dalHelper.CreateParameter("@StartDate", datePickerFrom.Value.Date, DbType.Date);
                            where += " StartDate>=@StartDate and ";
                        }

                        if (datePickerTo.Checked == true)
                        {
                            dalHelper.CreateParameter("@EndDate", datePickerTo.Value.Date, DbType.Date);
                        ...
                        reportForm = new ExternalTrainingReportForm(dt, reportFilterDataTable);
                    reportForm.Show();
                }
        ...
        private void cboRequestType_DropDown(object sender, EventArgs e)
        {
            try
            {
                ...
                Query query = new Query();
                requestTypes=this.requestTypesDataMapper.getAll(GlobalData.User.ID);
                foreach (ExcuseDutyRequestType requestType in requestTypes)
                {
                    cboSponsor.Items.Add(requestType.description);
                }
            }
            catch (Exception ex)
            {
        ...
        private void cmbInstitution_DropDown(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    cmbInstitution.Items.Add(school.Description);
                }
            }
            catch (Exception xi)
            {
        ...
        private TrainingSponsorDataMapper sponsorDataMapper;
        private IList<TrainingSponsor> lstsponsors;
        ...
        private void cboSponsor_DropDown(object sender, EventArgs e)
        {
            ...
            {
                cboSponsor.Items.Clear();
                sponsorDataMapper = new TrainingSponsorDataMapper();
                lstsponsors = sponsorDataMapper.getAll(true);
                foreach (var sponsor in lstsponsors)
                {
