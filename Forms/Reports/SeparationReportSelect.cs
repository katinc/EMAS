...
{
    public partial class SeparationReportSelect : Form
    {
        private IDAL dal;
        ...
        private IList<Termination> separations;
        private Form reportForm;
        private DALHelper dalHelper;
        private Company company;
        private int ctr;

        public SeparationReportSelect()
        {
            try
            {
                InitializeComponent();
                ...
                this.separations = new List<Termination>();
        ...
        private void cmbOption_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cmbOption.Text.ToLower() != "individual")
                {
                    ...
                    lblSeparationType.Visible = true;
                    cboSeparationType.Visible = true;
                }
                else
                {
                    ...
                    lblSeparationType.Visible = false;
                    cboSeparationType.Visible = false;
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (cmbOption.Text.Trim() == "Individual")
                {
                    if (ValidateFields())
                    {
                        ...
                        separations = dal.GetByCriteria<Termination>(query);
                        if (separations.Count <= 0)
                        {
                            MessageBox.Show("The selected staff's appointment is not terminated");
                        }
                        else
                        {
                            ...
                            reportForm = new SeparationReportForm(staffIDtxt.Text.Trim(), cboSeparationType.Text.Trim(), datePickerFrom.Checked, datePickerTo.Checked, datePickerFrom.Value, datePickerTo.Value, cboDepartment.Text.Trim(), cboUnit.Text.Trim(), cboGradeCategory.Text.Trim(), cboGrade.Text.Trim(), cboZone.Text.Trim());
                            reportForm.Show();
                        }
                    }
                }
        ...
        private void cboSeparationType_DropDown(object sender, EventArgs e)
        {
            ...
            {
                cboSeparationType.Items.Clear();
                separationTypes = dal.GetAll<SeparationType>();
        ...
        private void SeparationReportSelect_Load(object sender, EventArgs e)
        {
            try
            {
