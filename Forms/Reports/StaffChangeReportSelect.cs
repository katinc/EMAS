    ...
    public partial class StaffChangeReportSelect : Form
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (cboReportType.Text.ToLower().Trim() == "staff changes request report")
                    {
                        ...
                        reportForm = new StaffChangeRequestForm(staffIDtxt.Text.Trim(), datePickerFrom.Checked, datePickerTo.Checked, datePickerFrom.Value, datePickerTo.Value, cboDepartment.Text.Trim(), cboUnit.Text.Trim(), cboDatasets.Text);
                        reportForm.Show();
                    }
                    else if (cboReportType.Text.ToLower().Trim() == "staff changes request approval report")
                    {
                        if (reportForm != null && !reportForm.IsDisposed)
                        {
                            reportForm.Close();
                        ...
                        reportForm = new StaffChangeRequestApprovalForm(staffIDtxt.Text.Trim(), datePickerFrom.Checked, datePickerTo.Checked, datePickerFrom.Value, datePickerTo.Value, cboDepartment.Text.Trim(), cboUnit.Text.Trim(), cboDatasets.Text);
                        reportForm.Show();
                    }
                }
                else
        ...
        private void StaffChangeReportSelect_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                var datasets = GlobalData._context.EnforceDatasets.Where(u => u.IsActive == true).OrderBy(u=>u.Description).ToList();
                cboDatasets.Items.Add("ALL");

                foreach (var item in datasets)
                {
                    cboDatasets.Items.Add(item.Description);
                }

                //cboDatasets.DataSource = null;

                //cboDatasets.DataSource = datasets;
                //cboDatasets.DisplayMember = "Description";
                //cboDatasets.ValueMember = "ID";
                HideControls();
            }
            catch (Exception ex)
            {
        ...
        private void cboReportType_DropDown(object sender, EventArgs e)
        {
            try
            {
                ...
                cboReportType.Items.Add("Staff Changes Request Report");
                cboReportType.Items.Add("Staff Changes Request Approval Report");
