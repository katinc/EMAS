    ...
    public partial class LeaveRequest : Form
    {
        ...
        private void btnPreviewLetter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    dalHelper.CreateParameter("@StaffMainID", staffIDtxt.Text, DbType.String);
                    where += " where StaffID=@StaffMainID ";
                }

                string sql = string.Empty;

                if (leaveTypecmb.Text == "Annual Leave" && hasAnnualLeave())
                {
                    sql = "Select * from StaffPersonalInfo " + where + " order by StaffID";
                }

                var dataTable = new DataTable("StaffPersonalInfo");
                dataTable = dalHelper.ExecuteReader(sql);

                LeaveLetterApplicationForm form = new LeaveLetterApplicationForm(dataTable);
                form.Show();
            }
            catch (Exception ex)
            {
        ...
        string hasPendingLeaveActive(string staffID, int Year)
        {
            try
            {
                ...
                var leaveStatus = dalCommand.ExecuteScalar(" select top 1 case when recommended='false' then 'UNRECOMMENDED' when approved='false' then 'UNAPPROVED' else 'NOT_AVAILABLE' end AS LEAVE_ACTION_STATUS  from StaffLeave where staffID=@staffID and leaveYear=@leaveYear and  (Approved='false' or recommended='false') order by LeaveDate desc");
