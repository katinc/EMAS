    ...
    public partial class ConfirmationForm : Form
    {
        ...
        private void Clear()
        {
            try
            {
                ...
                confirmationDatePicker.ResetText();
        ...
        public void EditConfirmation(DataGridViewRow row)
        {
            try
            {
                Clear();
                ...
                gradeTextBox.Text = employee.Grade.Grade;
                specialtyTextBox.Text = employee.Specialty.Description;
                txtEmploymentDate.Text = employee.CurrentConfirmationDate.Value.Date.ToString("dd/MM/yyyy");
                confirmationID = int.Parse(row.Cells["gridID"].Value.ToString());
                confirmationDatePicker.Text = row.Cells["gridConfirmationDate"].Value.ToString();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (ValidateFields())
                {
                    ...
                    UpdateConfirmationsEntity();
                    if (editMode)
                    {
                        employee.StaffID = staffIDtxt.Text.Trim();
                        employee = dal.LazyLoadUnconfirmedByStaffID<Employee>(employee);
                        employee.CurrentConfirmationDate = confirmationDatePicker.Value.Date;
                        employee.EmploymentDate = confirmationDatePicker.Value;
                        employee.Confirmer = GlobalData.User.Name;
                        employee.CurrentConfirmationTime = confirmationDatePicker.Value;
                    ...
                    else
                    {
                        ...
                        employee = dal.LazyLoadUnconfirmedByStaffID<Employee>(employee);
                        employee.CurrentConfirmationDate = confirmationDatePicker.Value.Date;
                        employee.EmploymentDate = confirmationDatePicker.Value.Date;
                        employee.Confirmer = GlobalData.User.Name;
                        employee.CurrentConfirmationTime = confirmationDatePicker.Value;
                        employee.Confirmed = true;
                        dal.Update(employee);
                    ...
                    GlobalData.ProcessEdit(OldValues, controls, this, confirmationID, staffIDtxt.Text);
                    Clear();
                }
            }
            catch (Exception ex)
        ...
        private void UpdateConfirmationsEntity()
        {
            try
            {
                confirmation.ID = confirmationID;
                confirmation.Employee.StaffID = staffIDtxt.Text.Trim();
                confirmation.Employee.ID = staffCode;
                confirmation.ConfirmationDate = confirmationDatePicker.Value;
                confirmation.User.ID = GlobalData.User.ID;
                confirmation.Reason=reasonTextBox.Text;
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
        ...
        private bool ValidateFields()
        {
            ...
            try
            {
                if (confirmationDatePicker.Checked ==  false || confirmationDatePicker.Text.Trim() == string.Empty)
                {
                    result = false;
                    staffErrorProvider.SetError(confirmationDatePicker, "Please select the Confirmation Date");
                    confirmationDatePicker.Focus();
                }
                else if (confirmationDatePicker.Checked && !Validator.DateRangeValidator(confirmationDatePicker.Value, GlobalData.ServerDate, DateUnit.Day, DateRangeBoundary.UpperInclusive, 0))
                {
                    result = false;
                    staffErrorProvider.SetError(confirmationDatePicker, "Please Confirmation Date cannot be greater than Today");
                    confirmationDatePicker.Focus();
                }
            }
            catch (Exception ex)
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            try
            {
                ConfirmationView form = new ConfirmationView(this);
