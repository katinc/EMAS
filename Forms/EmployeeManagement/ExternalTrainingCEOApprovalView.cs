...
{
    public partial class ExternalTrainingCEOApprovalView : Form
    {
        private IDAL dal;
        ...
        public ExternalTrainingCEOApprovalView()
        {
            InitializeComponent();
            this.dal = new DAL();
            this.employeeGrades = new List<EmployeeGrade>();
            ...
            try
            {
                ...
                if (cmbQualification.Text != string.Empty)
                {
                   ...
                   sql += " HRRecommended=@HRRecommended and";
                    ...
                    sql += " CEOApproval=@CEOApproval and";
                    dalHelper.CreateParameter("@CEOApproval", false, DbType.Boolean);
        ...
        private void PopulateGrid()
        {
            ...
            foreach (ExternalTraining item in lstExternalTrainings)
            {
                ...
                gridViewInfo.Rows[ctr].Cells["gridHR"].Value =item.HR!=null? (item.HR.FirstName + " " + item.HR.OtherName).Trim() + " " + item.HR.Surname:string.Empty;
                gridViewInfo.Rows[ctr].Cells["gridHRDecision"].Value = item.HRDecision;
                gridViewInfo.Rows[ctr].Cells["gridHRComments"].Value = item.HRComment;
                gridViewInfo.Rows[ctr].Cells["gridHRAssessmentDate"].Value = item.HRAssessmentDate.ToShortDateString();

                gridViewInfo.Rows[ctr].Cells["gridCEO"].Value =item.CEO!=null? (item.CEO.FirstName + " " + item.CEO.OtherName).Trim() + " " + item.CEO.Surname:string.Empty;
                gridViewInfo.Rows[ctr].Cells["gridCEOComment"].Value = item.CEOComment;
                gridViewInfo.Rows[ctr].Cells["gridCEOApprovalDate"].Value = item.CEOApprovalDate.ToShortDateString();
        ...
        private void removeButton_Click(object sender, EventArgs e)
        {
            try
            {
                if (gridViewInfo.CurrentRow != null)
                {
                    ...
                    ExternalTrainingCEOApproval ApprovalForm = new ExternalTrainingCEOApproval(this,externalTraining);
