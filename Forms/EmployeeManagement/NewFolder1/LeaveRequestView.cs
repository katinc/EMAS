        ...
        void grid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (grid.CurrentRow != null)
                {
                    ...
                    var leaves = GlobalData._context.StaffLeaves.SingleOrDefault(u => u.ID == leave.ID);
                    var employees = GlobalData._context.StaffPersonalInfos.SingleOrDefault(u => u.StaffID == leave.Employee.StaffID);
                    ...
                    leave.Employee.Unit.Description = grid.CurrentRow.Cells["gridUnit"].Value.ToString();
                    ...
                    if (grid.CurrentRow.Cells["gridCurrentLeaveEndDate"].Value != null)
                    {
                        leave.Employee.CurrentLeaveEndDate = employees.CurrentLeaveEndDate;
                    }
                    if (grid.CurrentRow.Cells["gridAnnualLeaveProposedStartDate"].Value != null)
                    {
                        leave.Employee.AnnualLeaveProposedStartDate = employees.AnnualLeaveProposedStartDate;
                    }
                    if (grid.CurrentRow.Cells["gridAnnualLeaveProposedEndDate"].Value != null)
                    {
                        leave.Employee.AnnualLeaveProposedEndDate = employees.AnnualLeaveProposedEndDate;
                    }
                    ...
                    {
                        leave.StartDate = leaves.StartDate;
                    }
                    if (grid.CurrentRow.Cells["gridEndDate"].Value != null)
                    {
                        leave.EndDate = leaves.EndDate; 
                        //DateTime.Parse(grid.CurrentRow.Cells["gridEndDate"].ToString());
                    }
                    if (grid.CurrentRow.Cells["gridLeaveDate"].Value != null)
                    {
                        leave.LeaveDate = leaves.LeaveDate.Value;
                            
                            //DateTime.Parse(grid.CurrentRow.Cells["gridLeaveDate"].Value.ToString());
                    }
                    leave.Recommended = bool.Parse(grid.CurrentRow.Cells["gridRecommended"].Value.ToString());
                    leave.Approved = bool.Parse(grid.CurrentRow.Cells["gridApproved"].Value.ToString());
                ...
                {
                    MessageBox.Show("The Pending Leave Request is in Process,You cannot Edit");
                }
            }
            catch (Exception ex)
            {
                    ...
                    if (MessageBox.Show("Are you sure you want to remove " + grid.CurrentRow.Cells["gridStaffName"].Value.ToString() + "'s leave?", GlobalData.Caption, MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                    {
                        try
                        {
                            if (GlobalData.User.UserCategory.Description == "Intermediate" && GlobalData.User.ID == int.Parse(grid.CurrentRow.Cells["gridUserID"].Value.ToString()))
                            {
                                ...
                                leave.ID = int.Parse(grid.CurrentRow.Cells["gridID"].Value.ToString());
                                leave.Employee.StaffID = grid.CurrentRow.Cells["gridStaffId"].Value.ToString();
                                leave.NumberOfDays = Convert.ToInt32(grid.CurrentRow.Cells["gridNumberOfDays"].Value.ToString());
                                dal.Delete(leave);
                                var employees = GlobalData._context.StaffPersonalInfos.SingleOrDefault(u=>u.StaffID == leave.Employee.StaffID);
                                employees.LeaveTaken = employees.LeaveTaken - leave.NumberOfDays;
                                employees.LeaveBalance = employees.LeaveArrears + leave.NumberOfDays;
                                employees.LeaveArrears = employees.LeaveArrears + leave.NumberOfDays;
                            ...
                            else if (GlobalData.User.UserCategory.Description == "Basic" || GlobalData.User.UserCategory.Description == "Advance")
                            {
                                ...
                                leave.NumberOfDays = Convert.ToInt32(grid.CurrentRow.Cells["gridNumberOfDays"].Value.ToString());
                                var employees = GlobalData._context.StaffPersonalInfos.SingleOrDefault(u => u.StaffID == leave.Employee.StaffID);
                                GlobalData._context.Refresh(System.Data.Linq.RefreshMode.OverwriteCurrentValues, employees);
                                employees.LeaveTaken = employees.LeaveTaken - leave.NumberOfDays;
                                employees.LeaveBalance = employees.LeaveArrears + leave.NumberOfDays;
                                employees.LeaveArrears = employees.LeaveArrears + leave.NumberOfDays;
                                try
                                ...
                                {
                                    MessageBox.Show(ex.Message + "\nLeave not deleted,Contact your administrator");
                                    Logger.LogError(ex);
                                    return;
                                }
                                dal.Delete(leave);
        ...
        private void PopulateView(IList<Leave> leaves)
        {
            try
            {
                ...
                foreach (Leave leave in leaves)
                {
                    ...
                    if (grid.Rows[ctr].Cells["gridCurrentLeaveEndDate"].Value != null)
                    {
                        grid.Rows[ctr].Cells["gridCurrentLeaveEndDate"].Value = leave.Employee.CurrentLeaveEndDate.Value.ToString("dd/MM/yyyy");
                    }
                    
                    grid.Rows[ctr].Cells["gridLeaveArrears"].Value = leave.Employee.LeaveArrears;

                    if (grid.Rows[ctr].Cells["gridAnnualLeaveProposedStartDate"].Value != null && grid.Rows[ctr].Cells["gridAnnualLeaveProposedEndDate"].Value != null)
                    {
                        grid.Rows[ctr].Cells["gridAnnualLeaveProposedStartDate"].Value = leave.Employee.AnnualLeaveProposedStartDate.Value.ToString("dd/MM/yyyy");
                        grid.Rows[ctr].Cells["gridAnnualLeaveProposedEndDate"].Value = leave.Employee.AnnualLeaveProposedEndDate.Value.ToString("dd/MM/yyyy");
                    }
                    ...
                    grid.Rows[ctr].Cells["gridApprovalReason"].Value = leave.ApprovalReason;
                    grid.Rows[ctr].Cells["gridRecommendationReason"].Value = leave.RecommendationReason;
