...
namespace eMAS.Forms.EmployeeManagement
{
    public partial class DemotionView : Form
    {
        ...
        void grid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (grid.CurrentRow != null && demotionForm.CanEdit)
                {
                    ...
                    if (grid.CurrentRow.Cells["gridID"].Value != null)
                    {
                        if (GlobalData.QuestionMessage("Are you sure you want to Cancel the Demotion of the Staff " + "?") == DialogResult.Yes)
                        {
                            ...
                            if (GlobalData.User.UserCategory.Description == "Intermediate" && GlobalData.User.ID == int.Parse(grid.CurrentRow.Cells["gridUserID"].Value.ToString()))
                            {
                                ...
                                //dal.Delete(demotion);
                                GlobalData.ProcessDelete(this.Name, demotion.ID);
        ...
        private void PopulateView(IList<Promotion> demotions)
        {
            try
            {
                ...
                foreach (Promotion demotion in demotions)
                {
                    ...
                    grid.Rows[ctr].Cells["gridDemotionType"].Value = demotion.PromotionType.Description;
        ...
        private void GetData()
        {
            try
            {
                ...
                if (cboGrade.Text.Trim() != string.Empty)
                {
                    query.Criteria.Add(new Criterion()
                    {
                        Property = "PromotionView.cboGrade",
                        CriterionOperator = CriterionOperator.EqualTo,
                        Value = cboGrade.Text,
                        CriteriaOperator = CriteriaOperator.And
                    });
                ...
                if (cboDepartment.Text != string.Empty)
                {
                    query.Criteria.Add(new Criterion()
                        {
                            Property = "PromotionView.Department",
                            CriterionOperator = CriterionOperator.EqualTo,
        ...
        private void txtStaffID_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                grid.Rows.Clear();
                var company = GlobalData._context.CompanyInfos.First();
        ...
        private void datePickerDemotionDate_CloseUp(object sender, EventArgs e)
        {
            try
            {
                GetData();
