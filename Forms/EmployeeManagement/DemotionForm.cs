    ...
    public partial class DemotionForm : Form
    {
        ...
        void searchGrid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (searchGrid.CurrentRow != null)
                {
                    ...
                    //var demotion = GlobalData._context.Promotions.Where(u => u.StaffID == employee.StaffID && u.PromotionTypeID == 3 && u.Approved == false);
            ...
            try
            {
                if (ValidateFields())
                {
                    ...
                    else
                    {
                        if (datePickerEffectiveDate.Value.Date <= DateTime.Today.Date)
                        {
                            var demotionId = GlobalData._context.PromotionTypes.First(u => u.Description == "DEMOTION");
                            employee = dal.GetByID<Employee>(staffIDtxt.Text.Trim());
                            employee.CurrentPromotionDate = datePickerEffectiveDate.Value;
                            employee.GradeDate = datePickerEffectiveDate.Value;
                            employee.Grade.ID = promotion.Grade.ID;
                            ...
                            promotion.PromotionType.ID = int.Parse(demotionId.ID.ToString());
                            employee.OldBasicSalary = employee.BasicSalary;
                            employee.BasicSalary = promotion.NewSalary;
                            dal.Update(employee);
                            Query query = new Query();
                        ...
                        promotion.NotionalDate = DateTime.Today;
                        promotion.SubstantiveDate = DateTime.Today;
                        dal.Save(promotion);
                        dal.CommitTransaction();
                        Clear();
                    }
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                form.btnRemove.Enabled = CanEdit;
                form.Show();
            }
            catch (Exception ex)
            {
