    ...
    public partial class NewPromotion : Form
    {
        ...
        void cboGradeCategory_DropDown(object sender, EventArgs e)
        {
            try
            {
                ...
                cboGradeCategory.DataSource = gradeCategories;
        ...
        private void cboGradeCategory_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                ...
                var grades = GlobalData._context.EmployeeGradeViews.Where(u => u.Archived == false && u.GradeCategory == cboGradeCategory.Text).OrderBy(u => u.Description).ToList();
                ...
                //{
                //    cboGrade.DataSource = grades;
                //    cboGrade.DisplayMember = grade.Grade;
                //    cboGrade.ValueMember = grade.ID.ToString();
        ...
        private void Clear()
        {
            try
            {
                ...
                cboPromotionType.DataSource = null;
        ...
        public void EditPromotion(string promotionId)
        {
            try
            {
                Clear();
                editMode = true;
                //var promotionFull = GlobalData._context.PromotionViews.SingleOrDefault(p => p.ID.ToString() == promotionID);
                promotion = dal.GetByID<Promotion>(promotionId);
                employee.StaffID = promotion.Employee.StaffID;
                employee = dal.LazyLoadByStaffID<Employee>(employee);
                selectedEmployee = employee;
                string name = GlobalData.GetFullName(employee);
                nametxt.Text = name;
                staffIDtxt.Text = employee.StaffID;
                //staffCode = promotion.Employee.StaffID
                gendertxt.Text = employee.Gender;
                if (employee.Photo != null)
                {
                    pictureBox.Image = employee.Photo;
                ...
                cboPromotionType.Text = promotion.PromotionType.Description;
                cboGradeCategory_DropDown(this, EventArgs.Empty);
                cboGradeCategory.Text = promotion.GradeCategory.Description;
                cboGradeCategory_SelectionChangeCommitted(this,EventArgs.Empty);
                cboGrade.Text = promotion.Grade.Grade;
                if (promotion.Step.Description != null)
                {
                    cboStep_DropDown(this, EventArgs.Empty);
                    cboStep.Text = promotion.Step.Description;
                }
                if (promotion.Band.Description != null)
                {
                    cboBand_DropDown(this, EventArgs.Empty);
                    cboBand.Text = promotion.Band.Description;
                }
                
                datePickerEffectiveDate.Text = promotion.PromotionDate.ToString();
                datePickerNotional.Text = promotion.NotionalDate.ToString();
                datePickerSubstantive.Text = promotion.SubstantiveDate.ToString();
                txtNewSalary.Text = promotion.NewSalary.ToString();
                txtReason.Text = promotion.Reason == null ? string.Empty : promotion.Reason.ToString();
                if (GlobalData.User.UserCategory.Description == "Intermediate" && GlobalData.User.ID != int.Parse(promotion.User.ID.ToString()))
                {
            ...
            try
            {
                if (ValidateFields())
                {
                    ...
                    if (editMode)
                    {
                        ...
                        GlobalData.ProcessEdit(OldValues, controls, this, promotionID, staffIDtxt.Text);
                    }
                    else
                    {
                        dal.Save(promotion);
        ...
        private void ApprovePromotion(Promotion promotion)
        {
            try
            {
                employee.StaffID = promotion.Employee.StaffID;
                ...
                if (employee.ID != 0)
                {
                    ...
                    employee.Step.ID = promotion.Step.ID;
                    ...
                    promotion.ApprovedDate = DateTime.Today;
                    promotion.ApprovedTime = DateTime.Now;
        ...
        private void UpdatePromotionsEntity()
        {
            try
            {
                ...
                promotion.GradeCategory.ID = Convert.ToInt32(cboGradeCategory.SelectedValue);
                promotion.Grade.ID = Convert.ToInt32(cboGrade.SelectedValue);
        ...
        private bool ValidateFields()
        {
            ...
            try
            {
                ...
                {
                    result = false;
                    staffErrorProvider.SetError(cboStep, "Please select the Step");
                    cboStep.Focus();
                }
                //if (company.IsSalaryStructure == true && cboBand.Text.Trim() == string.Empty)
                //{
                //    result = false;
                //    staffErrorProvider.SetError(cboBand, "Please select the Level");
                //    cboBand.Focus();
                //}
        ...
        private void cboStep_DropDown(object sender, EventArgs e)
        {
            try
            {
                ...
                cboStep.DisplayMember = "Description";
        ...
        private void cboBand_DropDown(object sender, EventArgs e)
        {
            try
            {
                ...
                var bands = GlobalData._context.Bands.Where(u => u.Archived == false && u.Active).OrderBy(u => u.Description).ToList();

                cboBand.DataSource = bands;
                cboBand.DisplayMember = "Description";
                cboBand.ValueMember = "ID";
        ...
        private void setSalaryValue()
        {
            try
            {
                if (cboStep.SelectedIndex >= 0 && cboGrade.SelectedIndex >= 0 && cboGradeCategory.SelectedIndex >= 0)
                {
                    DataRow r = getStepSalary(gradeCategories[cboGradeCategory.SelectedIndex].ID, grades[cboGrade.SelectedIndex].ID, steps[cboStep.SelectedIndex].ID);
                    if (r != null)
                    {
                        txtNewSalary.Text = r["BasicSalary"].ToString();
                    ...
                    {
                        txtNewSalary.Text = selectedEmployee.BasicSalary.ToString();
                    }
                }
                else
                    txtNewSalary.Text = selectedEmployee.BasicSalary.ToString();
        ...
        private void cboPromotionType_DropDown(object sender, EventArgs e)
        {
            try
            {
                ...
                cboPromotionType.DataSource = promotionTypes;
                cboPromotionType.DisplayMember = "Description";
                cboPromotionType.ValueMember = "ID";
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
        ...
        private void cboBand_SelectionChangeCommitted(object sender, EventArgs e)
        {
            setSalaryValue();

        }

        private void cboStep_SelectionChangeCommitted(object sender, EventArgs e)
        {
            setSalaryValue();
        }
        ...
        {
            PromotionBulkForm form = new PromotionBulkForm();
            form.MdiParent = this.MdiParent;
            form.Show();
        }
    }
