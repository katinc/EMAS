    ...
    public partial class MedicalClaimsNew : Form
    {
                ...
                if (facility.Count() > 0)
                {
                    foreach (var item in facility)
                    {
                        if (item.Facility.ToString().Equals("KATH"))
                        {
                            ...
                            gridServiceAmount.HeaderText = unitPrice;
                            gridMedicineAmount.HeaderText = unitPrice;
                        }
                    }
                }
        ...
        private void Clear()
        {
            try
            {
                ...
                //Claims
                healthFacilityTypecmb.DataSource = null;
        ...
        private void healthFacilityTypecmb_DropDown(object sender, EventArgs e)
        {
            try
            {
                ...
                healthFacilityTypecmb.DataSource = healthFacilityTypes;
                healthFacilityTypecmb.DisplayMember = "Description";
                healthFacilityTypecmb.ValueMember = "ID";
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
        ...
        private void cboRelationship_DropDown(object sender, EventArgs e)
        {
            try
            {
                ...
                cboRelationship.DataSource = relationships;
                cboRelationship.DisplayMember = "Description";
                cboRelationship.ValueMember = "ID";
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
