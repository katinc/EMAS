...
{
    public partial class ExternalTrainingView : Form
    {
        private IDAL dal;
        ...
        public ExternalTrainingView(ExternalTrainingForm parentForm)
        {
            InitializeComponent();
            this.dal = new DAL();
            this.employeeGrades = new List<EmployeeGrade>();
        ...
        private void PopulateGrid()
        {
            try
            {
                ...
                foreach (ExternalTraining item in lstExternalTrainings)
                {
                    ...
                    gridViewInfo.Rows[ctr].Cells["gridSchool"].Value = item.School.Description;
                    ...
                    gridViewInfo.Rows[ctr].Cells["gridHODJustification"].Value =item.isJustified==true? item.Justification:string.Empty;
        ...
        private void gridViewInfo_DoubleClick(object sender, EventArgs e)
        {
          if (gridViewInfo.SelectedRows.Count > 0)
            {
                var selectedIndex = gridViewInfo.CurrentRow.Index;
                var externalTraining = externalTrainingMapper.getById(Convert.ToInt32(gridViewInfo.Rows[selectedIndex].Cells["gridID"].Value));
