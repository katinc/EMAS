    ...
    public class LeaveDataMapper
    {
        ...
        public void Save(object item)
        {
            try
            {
                ...
                dal.ExecuteNonQuery("INSERT INTO StaffLeave (StaffID,StaffCode,LeaveType,Status,StartDate,EndDate,LeaveDate,NumberOfDays,Reason,UserID,Institution,Programme,Duration) VALUES(@StaffID,@StaffCode,@LeaveType,@Status,@StartDate,@EndDate,@LeaveDate,@NumberOfDays,@Reason,@UserID,@Institution,@Programme,@Duration)");
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
        ...
        public IList<Leave> GetByCriteria(Query query1)
        {
            ...
            try
            {
                ...
                dal.ClearParameters();
                dal.CreateParameter("@Archived", leave.Archived, DbType.Boolean);
                string query = "select StaffLeaveView.*,StaffLeaveView.Firstname +' '+ StaffLeaveView.OtherName +' '+ StaffLeaveView.Surname as StaffName From StaffLeaveView ";
                string selectStatement = QueryTranslater.TranslateQuery(query, query1);
                selectStatement += "  StaffLeaveView.Archived ='False' Order by DateAndTimeGenerated DESC";
