    ...
    public class EmployeeDataMapper 
    {
        ...
        public void Update(object item)
        {
            ...
            try
            {
                ...
                dal.ExecuteNonQuery("Update StaffPersonalInfo Set AppointmentTypeID=@AppointmentTypeID,StatusID=@StatusID,DOCA=@DOCA,DOFA=@DOFA,DOB=@DOB,Firstname=@FirstName,Surname=@Surname,OtherName=@OtherName,JobTitleID=@JobTitleID,JobTitleDate=@JobTitleDate,ZoneID=@ZoneID,InZoneDate=@InZoneDate,DepartmentID=@DepartmentID,UnitID=@UnitID,Confirmer=@Confirmer,CurrentConfirmationTime=@CurrentConfirmationTime,ProbationApprover=@ProbationApprover,ProbationApprovedDate=@ProbationApprovedDate,ProbationApprovedTime=@ProbationApprovedTime,LeaveArrears=@LeaveArrears,AnnualLeave=@AnnualLeave,AnnualLeaveYear=@AnnualLeaveYear,AnnualLeaveDate=@AnnualLeaveDate,AnnualLeaveProposedStartDate=@AnnualLeaveProposedStartDate,AnnualLeaveProposedEndDate=@AnnualLeaveProposedEndDate,AnnualLeaveProposedDays=@AnnualLeaveProposedDays,CasualLeave=@CasualLeave,CasualLeaveDate=@CasualLeaveDate,LeaveTaken=@LeaveTaken,LeaveBalance=@LeaveBalance,IncrementDate=@IncrementDate,CurrentLeaveDate=@CurrentLeaveDate,OnLeave=@OnLeave,CurrentLeaveStartDate=@CurrentLeaveStartDate,CurrentLeaveEndDate=@CurrentLeaveEndDate,LeaveStatus=@LeaveStatus,OnLeaveWithPay=@OnLeaveWithPay,Sanctioned=@Sanctioned,CurrentSanctionDate=@CurrentSanctionDate,SanctionTypeID=@SanctionTypeID,Payment=@Payment,Terminated=@Terminated,TerminationDate=@TerminationDate,SeparationTypeID=@SeparationTypeID,CurrentPromotionDate=@CurrentPromotionDate,PromotionTypeID=@PromotionTypeID,BasicSalary=@BasicSalary,OldBasicSalary=@OldBasicSalary,IsInSalaryInfo=@IsInSalaryInfo,GradeID=@GradeID,GradeDate=@GradeDate,GradeCategoryID=@GradeCategoryID,CurrentTransferredDate=@CurrentTransferredDate,TransferType=@TransferType,TransferredInternally=@TransferredInternally,TransferredOut=@TransferredOut,TransferredIn=@TransferredIn,Confirmed=@Confirmed,CurrentConfirmationDate=@CurrentConfirmationDate,EmploymentDate=@EmploymentDate,AssumptionDate=@AssumptionDate Where StaffID=@StaffID");
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
        ...
        public Employee GetByID(object key)
        {
            ...
            try
            {
                ...
                string query = "SELECT StaffPersonalInfoView.*,REPLACE(StaffPersonalInfoView.MobileNo,'-','') as MobileNumber,StaffPersonalInfoView.SupervisorFirstName +' '+ StaffPersonalInfoView.SupervisorOtherName +' '+ StaffPersonalInfoView.SupervisorSurname as SupervisorName From StaffPersonalInfoView ";
                query += "WHERE StaffPersonalInfoView.StaffID=@StaffID And StaffPersonalInfoView.Archived=@Archived order BY StaffPersonalInfoView.Surname ASC";
        ...
        public Employee GetByPhoneNumber(object key)
        {
            ...
            try
            {
                ...
                dal.ClearParameters();
                dal.CreateParameter("@Archived", "False", DbType.String);
                dal.CreateParameter("@MobileNumber", key.ToString(), DbType.String);
                string query = "SELECT StaffPersonalInfoView.*,REPLACE(StaffPersonalInfoView.MobileNo,'-','') as MobileNumber From StaffPersonalInfoView ";
                query += "WHERE StaffPersonalInfoView.MobileNumber=@MobileNumber And StaffPersonalInfoView.Archived=@Archived order BY StaffPersonalInfoView.Surname ASC";
        ...
        private IList<Employee> BuildEmployeeFromData(DataTable table)
        {
            ...
            try
            {
                foreach (DataRow row in table.Rows)
                {
                    ...
                    //    FingerPrintID = row["FingerPrintID"] == DBNull.Value ? string.Empty : row["FingerPrintID"].ToString(),
                    ...
                    //    NickName = row["NickName"] == DBNull.Value ? string.Empty : row["NickName"].ToString(),
                    //    FileNumber = new FileNumber() { ID = row["FileNumberID"] == DBNull.Value ? 0 : int.Parse(row["FileNumberID"].ToString()), Description = row["FileNumber"] == DBNull.Value ? string.Empty : row["FileNumber"].ToString() },
                    //    PIN = row["PIN"] == DBNull.Value ? string.Empty : row["PIN"].ToString(),
                    //    IsInSalaryInfo = row["IsInSalaryInfo"] == DBNull.Value ? false : bool.Parse(row["IsInSalaryInfo"].ToString()),
                    //    OldBasicSalary = row["OldBasicSalary"] == DBNull.Value ? 0 : decimal.Parse(row["OldBasicSalary"].ToString()),
                    //    NationalID = row["NationalID"] == DBNull.Value ? string.Empty : row["NationalID"].ToString(),
                    //    MaidenName = row["MaidenName"] == DBNull.Value ? string.Empty : row["MaidenName"].ToString(),
                    ...
                    //    QualificationType = new QualificationType() { ID = row["QualificationID"] == DBNull.Value ? 0 : int.Parse(row["QualificationID"].ToString()), Description = row["Qualification"] == DBNull.Value ? string.Empty : row["Qualification"].ToString() },
                    ...
                    //    GradeOnFirstAppointment = new EmployeeGrade() { ID = row["GradeOnFirstAppointmentID"] == DBNull.Value ? 0 : int.Parse(row["GradeOnFirstAppointmentID"].ToString()), Grade = row["GradeOnFirstAppointment"] == DBNull.Value ? string.Empty : row["GradeOnFirstAppointment"].ToString() },
                    //    SalaryGrouping = row["SalaryGrouping"] == DBNull.Value ? string.Empty : row["SalaryGrouping"].ToString(),
                    //    Step = new Step() { ID = row["StepID"] == DBNull.Value ? 0 : int.Parse(row["StepID"].ToString()), Description = row["Step"] == DBNull.Value ? string.Empty : row["Step"].ToString() },
                    //    Band = new Band() { ID = row["BandID"] == DBNull.Value ? 0 : int.Parse(row["BandID"].ToString()), Description = row["Band"] == DBNull.Value ? string.Empty : row["Band"].ToString() },
                    ...
                    //    DisabilityType = new DisabilityType() { ID = row["DisabilityTypeID"] == DBNull.Value ? 0 : int.Parse(row["DisabilityTypeID"].ToString()), Description = row["DisabilityType"] == DBNull.Value ? string.Empty : row["DisabilityType"].ToString() },
                    //    DisabilityDate = row["DisabilityDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["DisabilityDate"].ToString()),
                    //    IsDisable = row["IsDisable"] is DBNull ? false : bool.Parse(row["IsDisable"].ToString()),

                    //    LicenceType = new LicenceType() { ID = row["LicenceTypeID"] == DBNull.Value ? 0 : int.Parse(row["LicenceTypeID"].ToString()), Description = row["LicenceType"] == DBNull.Value ? string.Empty : row["LicenceType"].ToString() },
                    //    LicenceNumber = row["LicenceNumber"] == DBNull.Value ? string.Empty : row["LicenceNumber"].ToString(),

                    //    Title = new Titles() { ID = row["TitleID"] == DBNull.Value ? 0 : int.Parse(row["TitleID"].ToString()), Description = row["Title"] == DBNull.Value ? string.Empty : row["Title"].ToString() },
                    //    Nationality = new Nationality() { ID = row["NationalityID"] == DBNull.Value ? 0 : int.Parse(row["NationalityID"].ToString()), Description = row["Nationality"] == DBNull.Value ? string.Empty : row["Nationality"].ToString() },
                    //    Town = new Town() { ID = row["TownID"] == DBNull.Value ? 0 : int.Parse(row["TownID"].ToString()), Description = row["Town"] == DBNull.Value ? string.Empty : row["Town"].ToString() },
                    //    HomeTown = new Town() { ID = row["HomeTownID"] == DBNull.Value ? 0 : int.Parse(row["HomeTownID"].ToString()), Description = row["HomeTown"] == DBNull.Value ? string.Empty : row["HomeTown"].ToString() },
                    //    TelNo = row["TelNo"] == DBNull.Value ? string.Empty : row["TelNo"].ToString(),
                    //    MobileNo = row["MobileNumber"] == DBNull.Value ? string.Empty : row["MobileNumber"].ToString(),


                    //    DOM = row["DOM"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["DOM"].ToString()),
                    //    Religion = new Religion() { ID = row["ReligionID"] == DBNull.Value ? 0 : int.Parse(row["ReligionID"].ToString()), Description = row["Religion"] == DBNull.Value ? string.Empty : row["Religion"].ToString() },
                    //    POB = new Town() { ID = row["PlaceOfBirthID"] == DBNull.Value ? 0 : int.Parse(row["PlaceOfBirthID"].ToString()), Description = row["PlaceOfBirth"] == DBNull.Value ? string.Empty : row["PlaceOfBirth"].ToString() },
                    //    CountryOfBirth = new Country() { ID = row["CountryOfBirthID"] == DBNull.Value ? 0 : int.Parse(row["CountryOfBirthID"].ToString()), Description = row["CountryOfBirth"] == DBNull.Value ? string.Empty : row["CountryOfBirth"].ToString() },
                    //    DistrictOfBirth = new District() { ID = row["DistrictOfBirthID"] == DBNull.Value ? 0 : int.Parse(row["DistrictOfBirthID"].ToString()), Description = row["DistrictOfBirth"] == DBNull.Value ? string.Empty : row["DistrictOfBirth"].ToString() },
                    //    RegionOfBirth = new HRBussinessLayer.System_Setup_Class.Region() { ID = row["RegionOfBirthID"] == DBNull.Value ? 0 : int.Parse(row["RegionOfBirthID"].ToString()), Description = row["RegionOfBirth"] == DBNull.Value ? string.Empty : row["RegionOfBirth"].ToString() },
                    //    NoOfChildren = row["NoOfChildren"] == DBNull.Value ? 0 : int.Parse(row["NoOfChildren"].ToString()),
                    //    Denomination = new Denomination() { ID = row["DenominationID"] == DBNull.Value ? 0 : int.Parse(row["DenominationID"].ToString()), Description = row["Denomination"] == DBNull.Value ? string.Empty : row["Denomination"].ToString() },
                    //    NHISNumber = row["NHISNumber"] == DBNull.Value ? string.Empty : row["NHISNumber"].ToString(),
                    //    HouseNumber = row["HouseNumber"] == DBNull.Value ? string.Empty : row["HouseNumber"].ToString(),
                    //    StreetName = row["StreetName"] == DBNull.Value ? string.Empty : row["StreetName"].ToString(),

                    //    ResidentialTown = new Town() { ID = row["ResidentialTownID"] == DBNull.Value ? 0 : int.Parse(row["ResidentialTownID"].ToString()), Description = row["ResidentialTown"] == DBNull.Value ? string.Empty : row["ResidentialTown"].ToString() },
                    //    ResidentialRegion = new HRBussinessLayer.System_Setup_Class.Region() { ID = row["ResidentialRegionID"] == DBNull.Value ? 0 : int.Parse(row["ResidentialRegionID"].ToString()), Description = row["ResidentialRegion"] == DBNull.Value ? string.Empty : row["ResidentialRegion"].ToString() },
                    //    ResidentialCountry = new Country() { ID = row["ResidentialCountryID"] == DBNull.Value ? 0 : int.Parse(row["ResidentialCountryID"].ToString()), Description = row["ResidentialCountry"] == DBNull.Value ? string.Empty : row["ResidentialCountry"].ToString() },
                    ...
                    //    Region = new HRBussinessLayer.System_Setup_Class.Region() { ID = row["RegionID"] == DBNull.Value ? 0 : int.Parse(row["RegionID"].ToString()), Description = row["Region"] == DBNull.Value ? string.Empty : row["Region"].ToString() },
                    //    ContactCountry = new Country() { ID = row["ContactCountryID"] == DBNull.Value ? 0 : int.Parse(row["ContactCountryID"].ToString()), Description = row["ContactCountry"] == DBNull.Value ? string.Empty : row["ContactCountry"].ToString() },
                    //    ContactHomeTown = new Town() { ID = row["ContactHomeTownID"] == DBNull.Value ? 0 : int.Parse(row["ContactHomeTownID"].ToString()), Description = row["ContactHomeTown"] == DBNull.Value ? string.Empty : row["ContactHomeTown"].ToString() },
                    //    //Photo = row["Image"] is DBNull ? null : Global.ArrayToImage((byte[])row["Image"]),
                    ...
                    //    SSNITContribution = row["SSNIT"] == DBNull.Value ? false : bool.Parse(row["SSNIT"].ToString()),
                    //    IncomeTaxContribution = row["IncomeTax"] == DBNull.Value ? false : bool.Parse(row["IncomeTax"].ToString()),
                    //    IsSusuPlusContribution = row["IsSusuPlusContribution"] == DBNull.Value ? false : bool.Parse(row["IsSusuPlusContribution"].ToString()),
                    //    SusuPlusContributionAmount = row["SusuPlusContributionAmount"] == DBNull.Value ? 0 : decimal.Parse(row["SusuPlusContributionAmount"].ToString()),
                    ...
                    //    WithholdingTaxFixedAmount = row["WithholdingTaxFixedAmount"] == DBNull.Value ? 0 : decimal.Parse(row["WithholdingTaxFixedAmount"].ToString()),
                    ...
                    //    SalaryType = new SalaryType() { ID = row["SalaryTypeID"] == DBNull.Value ? 0 : int.Parse(row["SalaryTypeID"].ToString()), Description = row["SalaryType"] == DBNull.Value ? string.Empty : row["SalaryType"].ToString() },

                    //    Probation = row["Probation"] == DBNull.Value ? false : bool.Parse(row["Probation"].ToString()),
                    //    ProbationStartDate = row["ProbationStartDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["ProbationStartDate"].ToString()),
                    //    ProbationEndDate = row["ProbationEndDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["ProbationEndDate"].ToString()),
                    //    SSNITNo = row["SSNITNo"] == DBNull.Value ? string.Empty : row["SSNITNo"].ToString(),
                    //    TIN = row["TIN"] == DBNull.Value ? string.Empty : row["TIN"].ToString(),

                    //    //They should not be null ,Check this if Error:Object Refernce is not instantiated
                    //    EngagementType = row["EngagementType"] is DBNull ? EngagementTypes.None.ToString() : row["EngagementType"].ToString(),
                    ...
                    //    ProbationStatus = row["ProbationStatus"] is DBNull ? ProbationStatuses.None.ToString() : row["ProbationStatus"].ToString(),
                    //    PaymentType = row["PaymentType"] == DBNull.Value ? string.Empty : row["PaymentType"].ToString(),
                    //    StaffStatus = new StaffStatus() { ID = row["StatusID"] == DBNull.Value ? 0 : int.Parse(row["StatusID"].ToString()), Description = row["Status"] == DBNull.Value ? string.Empty : row["Status"].ToString() },
                    ...
                    //    User = new User() { ID = row["UserID"] == DBNull.Value ? 0 : int.Parse(row["UserID"].ToString()), UserName = row["UserName"] == DBNull.Value ? string.Empty : row["UserName"].ToString() },
                    //    Archived = bool.Parse(row["Archived"].ToString()),

                    //    EngagementGradeOn = new EmployeeGrade() { ID = row["EngagementGradeIDOn"] == DBNull.Value ? 0 : int.Parse(row["EngagementGradeIDOn"].ToString()), Grade = row["EngagementGradeOn"] == DBNull.Value ? string.Empty : row["EngagementGradeOn"].ToString() },
                    //    EngagementGradeLeaving = new EmployeeGrade() { ID = row["EngagementGradeIDLeaving"] == DBNull.Value ? 0 : int.Parse(row["EngagementGradeIDLeaving"].ToString()), Grade = row["EngagementGradeLeaving"] == DBNull.Value ? string.Empty : row["EngagementGradeLeaving"].ToString() },
                    //    EngagementDateApplied = row["EngagementDateApplied"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["EngagementDateApplied"].ToString()),
                    //    EngagementEffectiveDate = row["EngagementEffectiveDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["EngagementEffectiveDate"].ToString()),
                    //    EngagementEndingDate = row["EngagementEndingDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["EngagementEndingDate"].ToString()),
                    //    EngagementContractOption = row["EngagementContractOption"] is DBNull ? string.Empty : row["EngagementContractOption"].ToString(),
                    //    EngagementAnnualSalary = row["EngagementAnnualSalary"] is DBNull ? 0 : decimal.Parse(row["EngagementAnnualSalary"].ToString()),
                    //    DOFA = row["DOFA"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["DOFA"].ToString()),
                    //    DOCA = row["DOCA"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["DOCA"].ToString()),
                    //    AssumptionDate = row["AssumptionDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["AssumptionDate"].ToString()),
                    ...
                    //    JobTitle = new JobTitle() { ID = row["JobTitleID"] == DBNull.Value ? 0 : int.Parse(row["JobTitleID"].ToString()), Description = row["JobTitle"] == DBNull.Value ? string.Empty : row["JobTitle"].ToString() },
                    //    OccupationGroup = new OccupationGroup() { ID = row["OccupationGroupID"] == DBNull.Value ? 0 : int.Parse(row["OccupationGroupID"].ToString()), Description = row["OccupationGroup"] == DBNull.Value ? string.Empty : row["OccupationGroup"].ToString() },
                    ...
                    //    InZoneDate = row["InZoneDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["InZoneDate"].ToString()),

                    //    ServerDate = DateTime.Parse(row["ServerDate"].ToString()),
                    //    ServerTime = DateTime.Parse(row["ServerTime"].ToString()),
                    ...
                    //    TerminationDate = row["TerminationDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["TerminationDate"].ToString()),
                    ...
                    //    LeaveStatus = row["LeaveStatus"] == DBNull.Value ? string.Empty : row["LeaveStatus"].ToString(),
                    //    OnLeaveWithPay = bool.Parse(row["OnLeaveWithPay"].ToString()),
                    ...
                    //    AnnualLeave = int.Parse(row["AnnualLeave"].ToString()),
                    ...
                    //    AnnualLeaveProposedStartDate = row["AnnualLeaveProposedStartDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["AnnualLeaveProposedStartDate"].ToString()),
                    //    AnnualLeaveProposedEndDate = row["AnnualLeaveProposedEndDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["AnnualLeaveProposedEndDate"].ToString()),
                    //    AnnualLeaveProposedDays = row["AnnualLeaveProposedDays"] == DBNull.Value ? 0 : int.Parse(row["AnnualLeaveProposedDays"].ToString()),
                    //    AnnualLeaveEntitlement = new AnnualLeaveEntitlement()
                    //    {
                    //        ID = row["AnnualLeaveEntitlementID"] == DBNull.Value ? 0 : int.Parse(row["AnnualLeaveEntitlementID"].ToString()),
                    //        CategoryOfPost = row["CategoryOfPost"] == DBNull.Value ? null : row["CategoryOfPost"].ToString(),
                    //        NumberOfDays = row["NumberOfDays"] == DBNull.Value ? 0 : int.Parse(row["NumberOfDays"].ToString()),
                    //        TypeOfGrade = row["TypesOfGrade"] == DBNull.Value ? null : row["TypesOfGrade"].ToString(),
                    //        Status = row["AnnualLeaveStatus"] == DBNull.Value ? null : row["AnnualLeaveStatus"].ToString(),
                    //    },
                    //    CasualLeave = int.Parse(row["CasualLeave"].ToString()),
                    //    CasualLeaveDate = row["CasualLeaveDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["CasualLeaveDate"].ToString()),

                    //    LeaveArrears = int.Parse(row["LeaveArrears"].ToString()),
                    //    LeaveBalance = int.Parse(row["LeaveBalance"].ToString()),
                    //    LeaveTaken = int.Parse(row["LeaveTaken"].ToString()),
                    //    Confirmed = bool.Parse(row["Confirmed"].ToString()),
                    //    CurrentConfirmationDate = row["CurrentConfirmationDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["CurrentConfirmationDate"].ToString()),
                    //    PromotionType = new PromotionType()
                    //    {
                    //        ID = row["PromotionTypeID"] == DBNull.Value ? 0 : int.Parse(row["PromotionTypeID"].ToString()),
                    //        Description = row["PromotionType"] == DBNull.Value ? string.Empty : row["PromotionType"].ToString()
                    //    },
                    //    CurrentPromotionDate = row["CurrentPromotionDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["CurrentPromotionDate"].ToString()),
                    ...
                    //        Description = row["SanctionType"] == DBNull.Value ? string.Empty : row["SanctionType"].ToString()
                    //    },
                    //    CurrentSanctionDate = row["CurrentSanctionDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["CurrentSanctionDate"].ToString()),
                    //    TransferType = row["TransferType"] == DBNull.Value ? string.Empty : row["TransferType"].ToString(),
                    //    TransferredOut = bool.Parse(row["TransferredOut"].ToString()),
                    //    TransferedIn = bool.Parse(row["TransferredIn"].ToString()),
                    //    TransferredInternally = bool.Parse(row["TransferredInternally"].ToString()),
                    ...
                    table = dal.ExecuteReader("Select * From StaffEducationHistoryView where StaffID ='" + employee.StaffID + "' and Archived='False'");
                    ...
                    table = dal.ExecuteReader("Select * From StaffProfessionHistoryView where StaffID ='" + employee.StaffID + "' and Archived='False'");
                    ...
                    {
                        Referee referee = new Referee(int.Parse(rowR["ID"].ToString()), rowR["Name"].ToString(), rowR["Address"].ToString(), rowR["Occupation"].ToString(), rowR["TelNo"].ToString(), rowR["Email"].ToString());
                        employee.Referees.Add(referee);
                    }

                    table = dal.ExecuteReader("Select * From StaffEmploymentHistoryView where StaffID ='" + employee.StaffID + "' and Archived='False'");
        ...
        private IList<Employee> BuildLazyLoadEmployeeData(DataTable table)
        {
            IList<Employee> employees = new List<Employee>();
            try
            {
                foreach (DataRow row in table.Rows)
                {
                    ...
                    //    Grade = new EmployeeGrade() { ID = row["GradeID"] == DBNull.Value ? 0 : int.Parse(row["GradeID"].ToString()), Grade = row["Grade"] == DBNull.Value ? string.Empty : row["Grade"].ToString() },
                    //    GradeDate = row["GradeDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["GradeDate"].ToString()),
                    //    GradeOnFirstAppointment = new EmployeeGrade() { ID = row["GradeOnFirstAppointmentID"] == DBNull.Value ? 0 : int.Parse(row["GradeOnFirstAppointmentID"].ToString()), Grade = row["GradeOnFirstAppointment"] == DBNull.Value ? null : row["GradeOnFirstAppointment"].ToString() },
                    //    SalaryGrouping = row["SalaryGrouping"] == DBNull.Value ? string.Empty : row["SalaryGrouping"].ToString(),
                    //    Step = new Step() { ID = row["StepID"] == DBNull.Value ? 0 : int.Parse(row["StepID"].ToString()), Description = row["Step"] == DBNull.Value ? string.Empty : row["Step"].ToString() },
                    //    Band = new Band() { ID = row["BandID"] == DBNull.Value ? 0 : int.Parse(row["BandID"].ToString()), Description = row["Band"] == DBNull.Value ? string.Empty : row["Band"].ToString() },
                    //    BasicSalary = row["BasicSalary"] == DBNull.Value ? 0 : decimal.Parse(row["BasicSalary"].ToString()),
                    ...
                    //    Payment = row["Payment"] == DBNull.Value ? false : bool.Parse(row["Payment"].ToString()),
                    ...
                    //    AnnualLeaveYear = row["AnnualLeaveYear"] == DBNull.Value ? 0 : int.Parse(row["AnnualLeaveYear"].ToString()),
                    //    AnnualLeaveDate = row["AnnualLeaveDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["AnnualLeaveDate"].ToString()),
                    //    AnnualLeaveProposedStartDate = row["AnnualLeaveProposedStartDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["AnnualLeaveProposedStartDate"].ToString()),
                    //    AnnualLeaveProposedEndDate = row["AnnualLeaveProposedEndDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["AnnualLeaveProposedEndDate"].ToString()),
                    //    AnnualLeaveProposedDays = row["AnnualLeaveProposedDays"] == DBNull.Value ? 0 : int.Parse(row["AnnualLeaveProposedDays"].ToString()),
                    ...
                    //    Confirmed = row["Confirmed"] == DBNull.Value ? false : bool.Parse(row["Confirmed"].ToString()),
                    //    CurrentConfirmationDate = row["CurrentConfirmationDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["CurrentConfirmationDate"].ToString()),
                    ...
                    //    Sanctioned = row["Sanctioned"] == DBNull.Value ? false : bool.Parse(row["Sanctioned"].ToString()),
                    //    SanctionType = new SanctionType()
                    //    {
                    //        ID = row["SanctionTypeID"] == DBNull.Value ? 0 : int.Parse(row["SanctionTypeID"].ToString()),
                    //        Description = row["SanctionType"] == DBNull.Value ? string.Empty : row["SanctionType"].ToString()
                    ...
                    //    CurrentSanctionDate = row["CurrentSanctionDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["CurrentSanctionDate"].ToString()),
                    //    TransferType = row["TransferType"] == DBNull.Value ? string.Empty : row["TransferType"].ToString(),
                    //    TransferredOut = row["TransferredOut"] == DBNull.Value ? false : bool.Parse(row["TransferredOut"].ToString()),
                    //    TransferedIn = row["TransferredIn"] == DBNull.Value ? false : bool.Parse(row["TransferredIn"].ToString()),
                    //    TransferredInternally = row["TransferredInternally"] == DBNull.Value ? false : bool.Parse(row["TransferredInternally"].ToString()),
                    //    CurrentTransferredDate = row["CurrentTransferredDate"] is DBNull ? null : (Nullable<DateTime>)DateTime.Parse(row["CurrentTransferredDate"].ToString()),
                    //};
                    employees.Add(employee);
                }
