    ...
    public partial class PayRoll_PaySlip : Form
    {
        ...
        {            
            try
            {
                ...
                if (cmbOption.Text.Trim().ToLower() == "pay roll")
                {
                    try
                    {
                        ...
                        if (status == PayRollStatus.Closed || status == PayRollStatus.Open)
                        {
                            ...
                            data.checkBoxOldFormat = checkBoxOldFormat.Checked;
                            if (checkBoxOldFormat.Checked == true)
                            {
                                if (reportForm != null && !reportForm.IsDisposed)
                                {
                                    reportForm.Close();
                                }
                                data.Month = monthComboBox.Text;
                ...
                else if (cmbOption.Text.Trim().ToLower() == "single pay slip")
                {
                    try
                    {
                        int paymentID = 0;
                        ...
                        {
                            MessageBox.Show("PayRoll is not generated for " + "Staff ID" + " " + staffIDtxt.Text + "( " + nametxt.Text + ")");
                            frm.Close();
                        }
                        else
                        {
                            ...
                            reportForm = new PaySlipReportForm(staffIDtxt.Text.Trim(), paymentID, monthComboBox.Text, yearComboBox.Text);
                            reportForm.Show();
                            frm.Close();
                        }
                    }
                ...
                //        cryRpt.Load(@"C:\EMAS-NCHS\Forms\Reports\PaySlipReport.rpt");
                ...
                //        }
                //        string paymentID = dalHelper.ExecuteScalar("select PaymentID from StaffSalaryPayments Where Month =" + GlobalData.GetMonth(monthComboBox.Text) + " And Year='" + yearComboBox.Text + "'").ToString();
                //        DataTable table = dalHelper.ExecuteReader("Select Distinct StaffID From StaffSalaryPayments Where PaymentID =" + paymentID);
                //        foreach (DataRow row in table.Rows)
                //        {
                //            //reportViewer.ReportSource = cryRpt;
                //            string staffID = row["StaffID"].ToString();
                //            cryRpt.RecordSelectionFormula = "{StaffSalaryPayments.StaffID}='" + row["StaffID"].ToString() + "' And {StaffSalaryPayments.PaymentID}=" + paymentID + " And {StaffSalaryPaymentsAllowancesAndDeductions.StaffID} ='" + row["StaffID"].ToString() + "' And {StaffSalaryPaymentsAllowancesAndDeductions.PaymentID} =" + paymentID;
        ...
        void cmbOption_DropDown(object sender, EventArgs e)
        {
            try
            {
                ...
                cmbOption.Items.Add("Pay Roll");
                cmbOption.Items.Add("Single Pay Slip");
        ...
        private void cmbOption_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (cmbOption.Text.ToLower() != "single pay slip")
                {
                    ...
                    checkBoxOldFormat.Visible = true;
                }
                else
                {
                    ...
                    checkBoxOldFormat.Visible = false;
        ...
        private static void RunReport(object value)
        {
            try
            {
                ...
                if (data.checkBoxOldFormat == true)
                {
                    data.report = new PayRollOldReportForm(data.Month, data.Year, data.Depart, data.Unit, data.GradeCat, data.Grade, data.Zone, frm);
                }
                else
                {
                    data.report = new PayRollReportForm(data.Month, data.Year, data.Depart, data.Unit, data.GradeCat, data.Grade, data.Zone);
                }
                //Use ShowDialog or Application.Run(data.report)
                //data.report.ShowDialog();
