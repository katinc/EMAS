            ...
            try
            {
                ...
                else if (cboFormat.Text.ToLower().Trim() == "master list")
                {
                    if (reportForm != null && !reportForm.IsDisposed)
                    {
                     ...
                     reportForm = new StaffProfileMasterListReportForm(datePickerFromDOFA.Checked, datePickerToDOFA.Checked, datePickerFromDOFA.Value, datePickerToDOFA.Value, datePickerFromAssumption.Checked, datePickerToAssumption.Checked, datePickerFromAssumption.Value, datePickerToAssumption.Value, datePickerFrom.Checked, datePickerTo.Checked, datePickerFrom.Value, datePickerTo.Value, cboDepartment.Text.Trim(), cboUnit.Text.Trim(), cboGradeCategory.Text.Trim(), cboGrade.Text.Trim(), cboZone.Text.Trim(), cboMechanised.Text.Trim(), cboLeave.Text.Trim(), cboGender.Text.Trim(), cboProbation.Text.Trim(), datePickerAsAtDate.Value.Date, datePickerAsAtDate.Checked, cboConfirmed.Text.Trim());
                    reportForm.Show();
                }
                else
                {
