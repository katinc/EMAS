    ...
    public class ApplicantDataMapper
    {
        ...
        public void Save(object item)
        {
            try
            {
                ...
                dal.ExecuteNonQuery("Insert Into Applicants(Surname,MiddleName,FirstName,VacancyID,ContactNo,EmailAddress,ContactAddress,ApplicationDate) Values(@Surname,@MiddleName,@FirstName,@VacancyID,@ContactNo,@EmailAddress,@ContactAddress,@ApplicationDate)");
                ...
                foreach (StaffDocument document in applicant.Documents)
                {
                    ...
                    dal.ExecuteNonQuery("Insert Into ApplicantDocuments(ApplicantID,DateCreated,Description,DocumentGroup,DocumentContent,Path) Values(@ApplicantID,@DateCreated,@Description,@DocumentGroup,@DocumentContent,@Path)");
                }
        ...
        public void Update(object item)
        {
            try
            {
                ...
                dal.CreateParameter("@ApplicationDate", applicant.ApplicationDate, DbType.DateTime);
               // dal.CreateParameter("@UserID", applicant.User.ID, DbType.Int32);
                dal.ExecuteNonQuery("Update Applicants Set Surname=@Surname,MiddleName=@MiddleName,FirstName=@FirstName,ContactNo=@ContactNo,EmailAddress=@EmailAddress,VacancyID=@VacancyID,ContactAddress=@ContactAddress,ApplicationDate=@ApplicationDate Where ID=@ID");
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
        ...
        public IList<StaffDocument> getApplicantDocuments(int ApplicantId)
        {
            var lstStaffDocuments = new List<StaffDocument>();
            try
            {
    
                dal.ClearParameters();
                dal.CreateParameter("@ApplicantId", ApplicantId, DbType.Int32);
                var dt = dal.ExecuteReader("select * from ApplicantDocumentView where ApplicantId=@ApplicantId");

                foreach (DataRow item in dt.Rows)
                {
                    var staffDoc = new StaffDocument()
                    {
                        DateCreated = DateTime.Parse(item["DateCreated"].ToString()),
                        Description = item["Description"].ToString(),
                        DocumentGroup = item["DocumentGroup"].ToString(),
                    ...
                    lstStaffDocuments.Add(staffDoc);
                }
            }
            catch (Exception xi)
            {
