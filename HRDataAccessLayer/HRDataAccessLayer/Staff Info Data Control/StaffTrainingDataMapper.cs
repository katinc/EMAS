    ...
    public class StaffTrainingDataMapper
    {
        ...
        public int Save(object item)
        {
            ...
            {   StaffTraining staffTraining = (StaffTraining)item;
                dal.ClearParameters();
                dal.CreateParameter("@StaffID", staffTraining.Employee.StaffID, DbType.String);
                //dal.CreateParameter("@StaffCode", staffTraining.Employee.ID.ToString(), DbType.String);
                dal.CreateParameter("@TrainingTypeID", staffTraining.TrainingType.ID, DbType.String);
                dal.CreateParameter("@ISTID", staffTraining.InServiceTraining.ID, DbType.String);
                ...
                dal.CreateParameter("@Organisers", staffTraining.Organisers.Description.ToString(), DbType.String);
                ...
                dal.CreateParameter("@LocationType", staffTraining.LocationType, DbType.String);
                dal.CreateParameter("@SponsorID", staffTraining.Sponsor.ID, DbType.String);
                ...
                //dal.ExecuteNonQuery("Insert Into StaffTraining(StaffID,TrainingTypeID,ISTID,StartDate,EndDate,Organisers,Venue,LocationTypeID,SponsorID,UserID,DateEntered) Values(@StaffID,@TrainingTypeID,@ISTID,@StartDate,@EndDate,@Organisers,@Venue,@LocationTypeID,@SponsorID,@UserID,@DateEntered); SELECT SCOPE_IDENTITY();");

                var newRowID = dal.ExecuteScalar("Insert Into StaffTraining(StaffID,TrainingTypeID,ISTID,StartDate,EndDate,Organisers,Venue,LocationTypeID,SponsorID,UserID,DateEntered) Values(@StaffID,@TrainingTypeID,@ISTID,@StartDate,@EndDate,@Organisers,@Venue,@LocationTypeID,@SponsorID,@UserID,@DateEntered); SELECT SCOPE_IDENTITY();");
        ...
        public void Update(StaffTraining staffTraining)
        {
            try
            {
                ...
                dal.CreateParameter("@TransportationFee", staffTraining.TransportationFee.ToString(), DbType.String);
                dal.CreateParameter("@TotalCost", staffTraining.TotalCost.ToString(), DbType.String);
                dal.CreateParameter("@LocationType", staffTraining.LocationType, DbType.Int32);
        ...
        public IList<StaffTraining> GetAll()
        {
            ...
            try
            {
                ...
                string query = "Select * from StaffTrainingView";
                ...
                foreach (DataRow row in table.Rows)
                {
                    ...
                    { 
                        ...
                        TrainingType = new TrainingType() { ID = int.Parse(row["TrainingTypeID"].ToString()), Description = row["TrainingType"].ToString() },
                        InServiceTraining = new InServiceTraining() { ID = int.Parse(row["ISTID"].ToString()), Description = row["InServiceTraining"].ToString() },
                        StartDate = DateTime.Parse(row["StartDate"].ToString()),
                        EndDate = DateTime.Parse(row["EndDate"].ToString()),
                        Organisers = new Organizers() { Description = row["Organisers"].ToString() },
                        CertificateAwarded = row["CertificateAwarded"].ToString(),
                        ...
                        Sponsor = new Sponsor() { ID = int.Parse(row["SponsorID"].ToString()), Description = row["TrainingSponsors"].ToString() },
        ...
        public StaffTraining GetByID(object key)
        {
            ...
            foreach (StaffTraining st in BuildStaffTrainingFromDatas(table))
            {
                staffTraining = st;
            }
        ...
        private IList<StaffTraining> BuildStaffTrainingFromDatas(DataTable table)
        {
            IList<StaffTraining> staffTrainings = new List<StaffTraining>();
            foreach (DataRow row in table.Rows)
            {
        ...
        public IList<StaffTraining> GetByCriteria(Query query1) 
        {
            IList<StaffTraining> staffTrainings = new List<StaffTraining>();
            try
            {
                ...
                selectStatement += " order by DateEntered desc";
                table = dal.ExecuteReader(selectStatement);
                foreach (StaffTraining staffTraining in BuildStaffTrainingFromData(table))
                {
                    staffTrainings.Add(staffTraining);
                }
        ...
        #region BuildStaffTraininngFromData
        private IList<StaffTraining> BuildStaffTrainingFromData(DataTable table)
        {
            ...
            try
            {
                foreach (DataRow row in table.Rows)
                {
                    StaffTraining training = new StaffTraining()
                    {
                        ID = int.Parse(row["ID"].ToString()),
                        Employee = new Employee()
                        {
                        ...
                        {
                            ID = int.Parse(row["TrainingTypeID"].ToString()),
                            Description = row["TrainingType"].ToString(),
                        },
                        Organisers = new Organizers()
                        {
                            Description = row["Organisers"].ToString()
                        },
                        Sponsor = new Sponsor()
                        {
                            ID = int.Parse(row["SponsorID"].ToString()),
                            Description = row["TrainingSponsors"].ToString(),
                        },
                        LocationType = row["LocationType"].ToString(),
                        Venue = row["Venue"].ToString(),
                        DateEntered = DateTime.Parse(row["DateEntered"].ToString()),
                        StartDate = DateTime.Parse(row["StartDate"].ToString()),
                        EndDate = DateTime.Parse(row["EndDate"].ToString()),
                        Hours = ( row["Hours"] == DBNull.Value) ? 0 : Convert.ToDecimal(row["Hours"].ToString()),
                        //int v = year < 0 ? 0 : 100;
                        CertificateAwarded = (row["CertificateAwarded"] == DBNull.Value) ? "" : row["CertificateAwarded"].ToString(),
                        Days = (row["Duration"] == DBNull.Value) ? 0 : Convert.ToInt32(row["Duration"].ToString()),
                        Program = (row["Course"] == DBNull.Value) ? string.Empty : row["Course"].ToString(),
                    };
                    staffTrainings.Add(training);
