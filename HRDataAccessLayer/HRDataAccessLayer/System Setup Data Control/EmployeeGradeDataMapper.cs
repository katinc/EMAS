    ...
    public class EmployeeGradeDataMapper
    {
        ...
        public void Save(object item)
        {
            try
            {
                ...
                dal.CreateParameter("@StartStep", employeeGrade.StartStep , DbType.Int32);
                dal.CreateParameter("@EndStep", employeeGrade.EndStep, DbType.Int32);

                dal.ExecuteNonQuery("INSERT INTO EmployeeGrades_Setup (CategoryID,Code,Description,GradeLevel,BasicSalary,Active,UserID,StartStep,EndStep)VALUES(@CategoryID,@Code,@Grade,@GradeLevel,@BasicSalary,@Active,@UserID,@StartStep,@EndStep)");
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
        ...
        public void Update(object item)
        {
            try
            {
                ...
                dal.ExecuteNonQuery("Update EmployeeGrades_Setup Set Code =@Code, Description=@Grade, GradeLevel=@GradeLevel, CategoryID=@CategoryID,BasicSalary =@BasicSalary, Active=@Active, UserID=@UserID, StartStep=@StartStep,EndStep=@EndStep Where ID =@ID");
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
            ...
            try
            {
                ...
                foreach (DataRow row in gradeTable.Rows)
                {
                    empGrade = new EmployeeGrade()
                    {
                        ...
                        StartStep = int.Parse(row["StartStep"].ToString()),
                        EndStep = int.Parse(row["EndStep"].ToString()),
            ...
            try
            {
                foreach (DataRow row in table.Rows)
                {
                    EmployeeGrade employeeGrade = new EmployeeGrade()
                    {
                        ...
                        Active = bool.Parse(row["Active"].ToString()),
                        Archived = bool.Parse(row["Archived"].ToString()),
                        StartStep =row["StartStep"]!=DBNull.Value? int.Parse(row["StartStep"].ToString()):0,
                        EndStep =row["EndStep"]!=DBNull.Value? int.Parse(row["EndStep"].ToString()):0,
                        User = new User() { ID = int.Parse(row["UserID"].ToString()), UserName = row["UserName"].ToString() }
                    };
