    ...
    public class BankBranchDataMapper
    {
        ...
        public void Save(object item)
        {
            try
            {
                ...
                dal.CreateParameter("@SortCode", bankBranch.SortCode, DbType.String);
                dal.CreateParameter("@UserID", bankBranch.User.ID, DbType.Int32);
                dal.CreateParameter("@Active", bankBranch.Active, DbType.Boolean);

                dal.ExecuteNonQuery("Insert Into BankBranches(Code,Description,SortCode,BankID,UserID,Active) Values(@Code,@Description,@SortCode,@BankID,@UserID,@Active)");
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
        ...
        public void Update(object item)
        {
            try
            {
                ...
                dal.ExecuteNonQuery("Update BankBranches Set Code=@Code,UserID=@UserID,Description=@Description,SortCode=@SortCode,BankID=@BankID,Active=@Active Where ID =@ID and Archived=@Archived");
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
        ...
        public IList<BankBranch> GetByCriteria(Query query1)
        {
            ...
            try
            {
                ...
                dal.ClearParameters();
                dal.CreateParameter("@Archived", bankBranch.Archived, DbType.Boolean);
                string query = "select BankBranchView.* from BankBranchView ";
                //query += "where  ";
                ...
                foreach (DataRow row in table.Rows)
                {
                    BankBranch bankBranch = new BankBranch()
                    {
                        ...
                        { 
                            ...
                            Description = row["BankName"].ToString(), 
                            Sortcode=row["SortCode"]!=DBNull.Value?row["SortCode"].ToString():string.Empty,
