...
namespace HRDataAccessLayer.System_Setup_Data_Control
{
    public class HolidaysDataMapper
    {
        ...
        public IList<Holiday> GetAll()
        {
            ...
            try
            { 
                DataTable table = dal.ExecuteReader("Select ID,Date,Description From Holidays2 order by Date desc");
        ...
        public IList<Holiday> GetAll(int Year)
        {
            IList<Holiday> holidays = new List<Holiday>();
            try
            {
                dal.ClearParameters();
                dal.CreateParameter("@Year",Year,DbType.Int32);
                DataTable table = dal.ExecuteReader("Select ID,Date,Description From Holidays2 where YEAR(Date)=@Year");
                foreach (DataRow row in table.Rows)
                {
                    holidays.Add(new Holiday()
                    {
