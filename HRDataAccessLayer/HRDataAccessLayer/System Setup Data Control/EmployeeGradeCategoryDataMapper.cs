    ...
    public class EmployeeGradeCategoryDataMapper
    {
        ...
        public void Save(object item)
        {
            try
            {
                ...
                dal.CreateParameter("@OverTimeRate", gradeCategory.OverTimeRate, DbType.String);
                dal.CreateParameter("@EncashmentRate", gradeCategory.LeaveEncashmentRate, DbType.String);
                dal.ExecuteNonQuery("INSERT INTO GradeCategory_Setup (Code,Description,Active,OverTimeRate,EncashmentRate,UserID)VALUES(@Code,@Description,@Active,@OverTimeRate,@EncashmentRate,@UserID)");
            }
            catch (Exception ex)
            {
        ...
        private IList<GradeCategory> BuildGradeCategoryFromData(DataTable table)
        {
            ...
            try
            {
                foreach (DataRow row in table.Rows)
                {
                    ...
                    var hiii = row["EncashmentRate"].ToString();
                    var heee = row["OverTimeRate"].ToString();
                    ...
                    GradeCategory gradeCategory = new GradeCategory()
                    {
                        ...
                        Archived = bool.Parse(row["Archived"].ToString()),
                        User = new User() { ID = int.Parse(row["UserID"].ToString()), UserName = row["UserName"].ToString() },
                        LeaveEncashmentRate = row["EncashmentRate"].ToString(),
                        OverTimeRate = row["OverTimeRate"].ToString(),
                    };
