    ...
    public class StaffSalaryHistoryDataMapper
    {
        ...
        public void Save(object item)
        {
            try
            {
                ...
                /*if(salaryInformation.salaryPaymentAccount!=null)
                    dal.CreateParameter("@SalaryPaymentAccountID", salaryInformation.salaryPaymentAccount.ID, DbType.Int32);
                else
                    dal.CreateParameter("@SalaryPaymentAccountID",DBNull.Value, DbType.Int32);*/
        ...
        #endregion
        public StaffSalaryHistory GetSalaryHistory(string staffID)
        {
            try
            {
                string query = "select * from StaffSalaryHistoryView where archived='false' and staffID=@staffID and InUse='true' order by ID";
                dal.ClearParameters();
                dal.CreateParameter("@staffID", staffID, DbType.String);
                var salaryInfoTable = dal.ExecuteReader(query);
                return BuildStaffSalaryHistoryFromData(salaryInfoTable).FirstOrDefault();
            }
            catch (Exception xi)
            {
                Logger.LogError(xi);
        ...
        public IList<SalaryPaymentAccount> GetAllSalaryPaymentAccounts()
        {
            List<SalaryPaymentAccount> lstSalaryPaymentAccounts = new List<SalaryPaymentAccount>();
            try
            {
                string query = "select * from SalaryPaymentAccounts where archived='false'";
                dal.ClearParameters();
                var dt = dal.ExecuteReader(query);
                foreach (DataRow dataRow in dt.Rows)
                {
                    var paymentAccount = new SalaryPaymentAccount();
                    paymentAccount.ID = dataRow["ID"] != DBNull.Value ? Convert.ToInt32(dataRow["ID"]) : 0;
                    paymentAccount.AccountName = dataRow["AccountName"] != DBNull.Value ? Convert.ToString(dataRow["AccountName"]) : string.Empty;
                    paymentAccount.AccountCode = dataRow["AccountCode"] != DBNull.Value ? Convert.ToString(dataRow["AccountCode"]) : string.Empty;
                    paymentAccount.Active = dataRow["Active"] != DBNull.Value ? Convert.ToBoolean(dataRow["Active"]) : paymentAccount.Active;
                    paymentAccount.Archived = dataRow["Archived"] != DBNull.Value ? Convert.ToBoolean(dataRow["Archived"]) : paymentAccount.Archived;

                    lstSalaryPaymentAccounts.Add(paymentAccount);
                }
            }
            catch (Exception xi)
            {
            ...
            return lstSalaryPaymentAccounts;
        }
            ...
            try
            {
                foreach (DataRow row in table.Rows)
                {
                    StaffSalaryHistory salaryInfo = new StaffSalaryHistory()
                    {
                        ...
                        EndDate = row["EndDate"] == DBNull.Value ? null : (Nullable<DateTime>)DateTime.Parse(row["EndDate"].ToString())

                        /* table does notexist
                        salaryPaymentAccount=new SalaryPaymentAccount(){
                            ID=row["SalaryPaymentAccountID"]!=DBNull.Value?Convert.ToInt32(row["SalaryPaymentAccountID"]):0,
                            AccountName=row["SalaryPaymentAccountName"]!=DBNull.Value ? Convert.ToString(row["SalaryPaymentAccountName"]):string.Empty,
                            AccountCode=row["SalaryPaymentAccountCode"]!=DBNull.Value ? Convert.ToString(row["SalaryPaymentAccountCode"]):string.Empty,
                            Active=row["SalaryPaymentAccountActive"]!=DBNull.Value ? Convert.ToBoolean(row["SalaryPaymentAccountActive"]):true,
                            Archived=row["SalaryPaymentAccountArchived"]!=DBNull.Value ? Convert.ToBoolean(row["SalaryPaymentAccountArchived"]):false,
                        }*/
                    };
